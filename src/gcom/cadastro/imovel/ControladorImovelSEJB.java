...
//		 ------------ REGISTRAR TRANSA��O ----------------
	...
	/**
	 * Insere a Liga��o de Esgoto quando o im�vel � inserido com o indicador de
	...
	/**
	 * Atualiza a Liga��o de Esgoto quando o im�vel � inserido com o indicador de
	 * faturamento igual a sim
	 *
	 * @author Rafael Corr�a, Raphael Rossiter
	 ...
	 * 
	 * @param colecaoClienteImovel Cole��o Cliente Imovel
	...
	/**
	 * Verifica se ImovelSubCategoria existe na Cole��o de SubCategoria do Imovel
	 * Author: Rafael Santos Data: 01/02/2006
	 * 
	 * @param colecaoSubCategoria Cole��o Imovel Sub Categoria
	 ...
	 * < <Descri��o do m�todo>>
	 * 
	 * @param imovelSubcategoria Descri��o do par�metro
	 * @throws ControladorException
	 */
	 ...
	 * < <Descri��o do m�todo>>
	 * 
	 * @param objeto      Descri��o do par�metro
	 * @param condicional Descri��o do par�metro
	 * @param id          Descri��o do par�metro
	 * @throws ControladorException
	 */
	...
	public void removerImovel(String[] ids, Usuario usuarioLogado) throws ControladorException {
		...
		if (ids.length > 0 && ids != null) {
			for (int i = 0; i < ids.length; i++) {
				...
				// Verificar se o imovel j� foi atualizada por outro usu�rio durante esta
				// atualiza��o
				 ...
				 * [FS0017] Registrar Fim de Rela��o do(s) Cliente(s) com o Im�vel (Caso existam
				 * clientes com rela��o ativa com o im�vel, o sistema dever� encerrar esta
				 * rela��o, atualizando a data do t�rmino da rela��o e seu motivo de
				 * encerramento)
				 */
	 ...
	 * @param lote             Descri��o do par�metro
...
//	}
	...
	/**
	 * [UC0123] - Obter Descri�oes da Categoria do Im�vel Autor: S�vio Luiz Data:
	 * 27/12/2005
	 */
	 ...
	 * categorias do im�vel
	 *
	 * @author Rafael Santos, Ana Maria
	 * @date 29/12/2005, 17/11/2008
	 * @param colecaoCategorias Cole��o de Categorias
	 * @param valor             Valor
...
//	 }
	...
	/**
	 * Atualiza o id da cobran�a situa��o tipo da tabela im�vel com o id da situa��o
	 ...
	 * [UC0265] Inserir Pagamentos
	 * 
	 * Verifica se o usu�rio informou o im�vel ou o cliente, n�o pode existir os doi
	 * nem nenhum
	 * 
...
//				 Se o consumo medio pra o maior anoMesReferencia estiver fora do intervalo informado no filtro
				// a linha da cole��o � removida para n�o exibir no relatorio dados fora dos
...
//				.add(imovelRelatorioHelper);
	...
	/**
	 * O m�todo abaixo realiza uma pesquisa em imovel e retorna os campos
	 * necess�rios para a cria��o da inscri��o do im�vel para exibi��o, independente
	 * do im�vel ter sido exclu�do ou n�o.
	 * 
	...
	/**
	 * 
	 * Esse m�todo � usado para fzazer uma pesquisa na tabela im�vel e confirmar se
	 * o id passado � de um im�vel exclu�do(idExclusao)
	 * 
	 * Fl�vio Cordeiro
	 * 
	...
	/**
	 * Permite pesquisar entidades beneficentes [UC0389] Inserir Autoriza��o para
	 * Doa��o Mensal
	 * 
	 * @author C�sar Ara�jo
	 * @date 30/08/2006
	 ...
	 * @param idEntidadeBeneficente - C�digo da entidade beneficente
	 * @return ImovelDoacao - Retorna um im�vel doa��o caso a combina��o de im�vel e
	...
	/**
	 * Permite atualizar as informa��es do im�vel doa��o [UC0390] Manter Autoriza��o
	 * para Doa��o Mensal
	 * 
	...
	public void atualizarImovelDoacao(ImovelDoacao imovelDoacao, Usuario usuarioLogado) throws ControladorException {
		...
		try {
			...
			/*******************************************************************
			 * * Avalia se a data de altera��o do im�vel doa��o na base de dados � mais
			 * recente do que o registro em edi�ao
			 ******************************************************************/
			...
			if (!getControladorAcesso().verificarAcessoAbrangencia(abrangencia)) {
				...
				/**
				 * * Seta a nova data para o registro e chama o m�todo para atualizar os dados
				 * **
				 */
	...
	/**
	 * Verifica a exist�ncia do hid�metro de acordo com tipo de medi��o informado
	...
	public void validarExistenciaHidrometro(Imovel imovel, Integer medicaoTipo) throws ControladorException {
		...
		if (MedicaoTipo.LIGACAO_AGUA.equals(medicaoTipo)) {
			/*
			 * Verificar se a situa��o da liga��o de �gua do im�vel est� diferente de
			 * POTENCIAL e FACT�VEL. Caso contr�rio, exibir a mensagem:
			 * "Liga��o de �gua do im�vel <<imov_id>> est� <<last_dsligacaoaguasituacao>>" e
			...
			/*
			 * Caso o tipo de medi��o seja igual � "Liga��o de �gua" e n�o exista hidr�metro
			 * instalado na liga��o de �gua, exibir a mensagem: "N�o existe hidr�metro
			 * instalado na liga��o de �gua deste im�vel"
			 */
	 ...
	 * [UC0368] Atualizar Instala��o do Hidr�metro
	 * 
	 * [FS0001] - Verificar a exist�ncia da matr�cula do im�vel [FS0002] - Verificar
	 * a situa��o do im�vel
	 * 
	 * @author lms
	 * @created 19/07/2006
...
//			}
	...
	/**
	 * Pesquisa a cole��o de vencimento alternativos do imovel [UC0473] Consultar
	...
	/**
	 * Pesquisa a cole��o de Debitos Automaticos do imovel [UC0473] Consultar Imovel
	 ...
	 * logradouroTipo = null; // Descri��o logradouro tipo if (array[22] != null) {
		...
		if (idsImovelCobrancaSituacao != null) {
			...
			if (colecaoImovelCobrancaSituacao != null && !colecaoImovelCobrancaSituacao.isEmpty()) {
				...
				while (iterImovelCobrancaSituacao.hasNext()) {
					...
					// 3.1.4. Caso o intervalo de refer�ncia dos d�bitos associados � situa��o de
					// cobran�a
	 ...
	 * 
	 * [UC0251] Gerar Atividade de A��o de Cobran�a [SB0001] Gerar Atividade de A��o
	 * de Cobran�a para os Im�veis do Cliente
	 * 
	 ...
	 * 
	 * Utilizado no [UC0251] Gerar Atividade de A��o de Cobran�a [SB0001] Gerar
	 * Atividade de A��o de Cobran�a para os Im�veis do Cliente
	 * 
	 * @param idRota
	 ...
	 * 
	 * Verifica se j� existe outro im�vel ou economia com o mesmo n�mero de IPTU no
	 * mesmo munic�pio
	 * 
	 * @date 18/01/2007
	 * @author Rafael Corr�a
	 ...
	 * 
	 * Verifica se j� existe outro im�vel ou economia com o mesmo n�mero de contrato
	 * da companhia el�trica
	 * 
	 * @date 18/01/2007
	 * @author Rafael Corr�a
	 * @throws ControladorException
	 ...
	 * 
	 * Pequisa o identificador de cobranca de acr�scimo pro impontualidade para o
	 * im�vel do cliente respons�vel.
	 * 
	 * @author Pedro Alexandre
	 * @date 26/02/2007
	 * 
...
//		}
	 ...
	 * 
	 * Seleciona a lista de im�veis que esteja com a situa��o de �gua ligado em
	 * an�lise.
	 * 
	 * @author Yara Taciane
	 * @date 23/05/2008
	...
	public ImovelAbaLocalidadeRetornoHelper validarImovelAbaLocalidade(ImovelAbaLocalidadeHelper helper)
		 ...
		 * tabela SISTEMA_PARAMETROS) O campo face da quadra ser� obrigat�rio;
		 */
		SistemaParametro sistemaParametro = this.getControladorUtil().pesquisarParametrosDoSistema();
	...
	public ImovelAbaEnderecoRetornoHelper validarImovelAbaEndereco(ImovelAbaEnderecoHelper helper)
		...
		/*
		 * [FS0034] Validar dados cadastrais do Munic�pio Autor: Hugo Leonardo Data:
		 * 17/01/2011
		 */
	...
	public ImovelAbaCaracteristicasRetornoHelper validarImovelAbaCaracteristicas(ImovelAbaCaracteristicasHelper helper)
		...
		if (areaConstruida != null && !areaConstruida.equals("")) {
			...
			if (pesquisa != null && !pesquisa.isEmpty()) {
				...
				// throw new ControladorException("atencao.pesquisa_inexistente", null, "�rea
				// Constru�da");
			}
		...
		if (imovelAtualizar != null) {
			 ...
			 * INDICADOR_REDE_ESGOTO DISTRITO_OPERACIONAL e BACIA ser�o obtidos a partir da
			 * face.
			 */
			IntegracaoQuadraFaceHelper integracaoQuadraFace = null;
			SistemaParametro sistemaParametro = this.getControladorUtil().pesquisarParametrosDoSistema();
	...
	/**
	 * 
	 * Autor: S�vio Luiz Data: 07/05/2009
	 * 
	...
	public void verificarIndicadorNomeConta(Integer idImovel) throws ControladorException {
		...
		if (colecaoClienteImovel != null && !colecaoClienteImovel.isEmpty()) {
			...
			if (!nomeContaInformado) {
				...
				// ATUALIZANDO O CLIENTE DO TIPO USU�RIO PARA QUE SEU NOME SEJA COLOCADO NA
				// CONTA
				try {
	...
	/**
	 * [UC0074] Alterar Inscri��o de Im�vel [FS0010] � Verificar Duplicidade de
	 * Inscri��o
	 * 
	 * @author Arthur Carvalho
	 * @date 19/09/2010
	 ...
	 * 
	 * M�todo que verifica se um determinado im�vel possui faturamento ativo de �gua
	 * 
	 * @param imovel
	 * @return
	 */
	 ...
	 * Pamela Gatinho e Adriana Muniz 21/08/2012
	 * 
	 * M�todo que verifica se um determinado im�vel possui faturamento ativo de
	 * esgoto
	 * 
	 * @param imovel
	 * @return
	 ...
	 * Pamela Gatinho e Adriana Muniz 21/08/2012
	 * 
	 * M�todo que retorna a colecao de categoria ou subcategoria, e suas respectivas
	...
	public void retirarImovelTarifaSocial(Integer motivoExclusao, Imovel imovel, String observacaoRetira)
		...
		try {
			...
			if (imovel.getFaturamentoSituacaoTipo() != null) {
				// Caso a situa��o de faturamento(FTST_ID) do im�vel na tabela IMOVEL diferente
				// de NULO
			...
			// O sistema atualiza o perfil, situa��o de faturamento e consumo tarifa do
			// im�vel na tabela CADASTRO.IMOVEL
