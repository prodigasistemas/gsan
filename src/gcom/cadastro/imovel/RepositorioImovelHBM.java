...
public class RepositorioImovelHBM implements IRepositorioImovel {
	...
	public Imovel obterImovelPorId(Integer idImovel) throws ErroRepositorioException {
		...
		try {
			return (Imovel) session.createQuery("select i from Imovel i where i.id = :id").setParameter("id", idImovel)
					.uniqueResult();
		} catch (Exception e) {
	 ...
	 * 
	 * 
	 * UC-0355] - Efetuar Corte de Liga�� de �gua [SB001] Atualizar Im�vel
	 * 
		...
		try {
			...
			Iterator iterator = session.createQuery(
					...
					"from gcom.cadastro.cliente.ClienteImovelEconomia "
							...
							+ "and dataFimRelacao is null")
					.setInteger(

							"clienteImovelEconomia", id)
					.iterate();
	...
	public Collection pesquisarImovel(Integer idLocalidade,
			...
			Integer idSetorComercial, Integer idQuadra, Integer quadraFace, Short lote,
		...
		try {
			...
			if (quadraFace != null) {
				consulta = consulta + " and imovel.quadraFace.id = " + quadraFace;
			}
	...
	public void atualizarImovelInscricao(Imovel imovel)
		...
		try {
			...
			if (imovel.getQuadraFace().getId() != null) {
				update = "update gcom.cadastro.imovel.Imovel set "
						+ "loca_id = :loca, stcm_id = :stcm, qdra_id = :qdra,qdfa_id =:qdra_fa_id, imov_nnlote = :nnlote, "
						+ "imov_nnsublote = :nnsublote where imov_id = :imov";

				session.createQuery(update).setInteger("loca", imovel.getLocalidade().getId().intValue())
						.setInteger("stcm", imovel.getSetorComercial().getId().intValue())
						.setInteger("qdra", imovel.getQuadra().getId().intValue())
						.setInteger("qdra_fa_id", imovel.getQuadraFace().getId().intValue())
						.setInteger("nnlote", new Short(imovel.getLote()).intValue())
						.setInteger("nnsublote", new Short(imovel.getSubLote()).intValue())
						.setInteger("imov", imovel.getId().intValue()).executeUpdate();
			} else {
				update = "update gcom.cadastro.imovel.Imovel set "
	...
	public Integer pesquisarObterQuantidadeEconomias(Imovel imovel) throws ErroRepositorioException {
		...
		try {
			String consulta = "select sum(imsc.quantidadeEconomias) "
					+ "from gcom.cadastro.imovel.ImovelSubcategoria as imsc " + "inner join imsc.comp_id.imovel "
					+ "where imsc.comp_id.imovel.id = :imovelId ";

			Short economias = (Short) session.createQuery(consulta).setInteger("imovelId", imovel.getId().intValue())
					.uniqueResult();
	...
	public Cep pesquisarCepImovel(Imovel imovel)
		...
		try {
			...
			consulta = "select cep " + "from Imovel imov " + "inner join imov.logradouroCep.cep cep "
					+ "where imov.id = :imovelId ";

			retorno = (Cep) session.createQuery(consulta).setInteger("imovelId", imovel.getId().intValue())
	...
	public Collection pesquisarObterQuantidadeEconomiasCategoria(Integer imovel, Integer categoriaId)
			throws ErroRepositorioException {
		...
		try {
			consulta.append("select c.id, c.descricao, c.consumoEstouro, ")
					.append("c.vezesMediaEstouro, sum(isb.quantidadeEconomias), ").append("isb.comp_id.imovel.id, ")
					.append("c.consumoAlto, ").append("c.mediaBaixoConsumo, ").append("c.vezesMediaAltoConsumo, ")
					.append("c.porcentagemMediaBaixoConsumo,").append("c.descricaoAbreviada, ")
					.append("c.numeroConsumoMaximoEc, ").append("c.indicadorCobrancaAcrescimos, ")
					.append("c.fatorEconomias, ").append("c.categoriaTipo.id, ").append("c.categoriaTipo.descricao, ")
					.append("c.numeroConsumoMaximoEc ").append("from ImovelSubcategoria isb ")
					.append("inner join isb.comp_id.subcategoria sb ").append("inner join sb.categoria c ")
					.append("inner join c.categoriaTipo ct ").append("where isb.comp_id.imovel.id = :imovelId ");
			...
			Query query = session.createQuery(consulta.toString()).setInteger("imovelId", imovel.intValue());
...
//							"left join imovel.cobrancaSituacao ";
		...
		if (!colecaoImovel.isEmpty()) {
			...
			if (arrayImovel[8] != null
				...
				if (imovelRelatorioHelper.getIndicadorImovelCondominio() == 1) {
					...
					imovelRelatorioHelper
							...
							.setIndicadorImovelCondominioDescricao("N�o");
			...
			if (imovelRelatorioHelper.getIndicadorExistenciaCavaletePoco() == 1) {
				...
				imovelRelatorioHelper
						...
						.setIndicadorExistenciaCavaletePocoDescricao("N�o");
		...
		if (!colecaoLigacaoAgua.isEmpty()) {
			...
			if (imovelRelatorioHelper.getIndicadorExistenciaCavaleteAgua() == 1) {
				...
				imovelRelatorioHelper
						...
						.setIndicadorExistenciaCavaleteAguaDescricao("N�o");
	...
	public Collection pesquisarImovelSituacaoEspecialFaturamento(String valor,
		...
		if (valor.equals("COM") && ((helper.getIndicadorComercial() == null
		...
		} else if (!valor.equals("COM") && ((helper.getIndicadorComercial() == null
				|| helper.getIndicadorComercial().toString().equals("" + ConstantesSistema.NAO))
				&& (helper.getIndicadorIndustrial() == null
						|| helper.getIndicadorIndustrial().toString().equals("" + ConstantesSistema.NAO))
				&& (helper.getIndicadorPublico() == null
			...
			consulta = "select imovel.id, imovel.ultimaAlteracao  " + "from Imovel imovel "
		...
		if (!subLoteOrigem.equalsIgnoreCase("") && !subLoteDestino.equalsIgnoreCase(""))
			consulta += " imovel.subLote between " + new Integer(subLoteOrigem) + " and " + new Integer(subLoteDestino)
					+ " and";

		if ((codigoRotaInicial != null && !codigoRotaInicial.equals(""))
				&& (codigoRotaFinal != null && !codigoRotaFinal.equals(""))) {
			consulta = consulta + " rota.codigo between " + new Integer(codigoRotaInicial) + " and "
					+ new Integer(codigoRotaFinal) + " and";
		}

		if ((sequencialRotaInicial != null && !sequencialRotaInicial.equals(""))
				&& (sequencialRotaFinal != null && !sequencialRotaFinal.equals(""))) {
			consulta = consulta + " imovel.numeroSequencialRota between " + new Integer(sequencialRotaInicial) + " and "
					+ new Integer(sequencialRotaFinal) + " and";
		}

		System.out.println(Util.removerUltimosCaracteres(consulta, 4));

		try {
			return session.createQuery(Util.removerUltimosCaracteres(consulta, 4))
	...
	public Collection pesquisarImovelSituacaoEspecialCobranca(String valor,
		...
		// Escolha de que tipo de consulta ser�

		if (valor.equals("COM") && (situacaoEspecialCobrancaHelper.getIdCategoria() == null
				&& ((situacaoEspecialCobrancaHelper.getIndicadorComercial() == null || situacaoEspecialCobrancaHelper
						.getIndicadorComercial().toString().equals("" + ConstantesSistema.NAO))
						&& (situacaoEspecialCobrancaHelper.getIndicadorIndustrial() == null
								|| situacaoEspecialCobrancaHelper.getIndicadorIndustrial().toString()
										.equals("" + ConstantesSistema.NAO))
						&& (situacaoEspecialCobrancaHelper.getIndicadorPublico() == null
								|| situacaoEspecialCobrancaHelper.getIndicadorPublico().toString()
										.equals("" + ConstantesSistema.NAO))
						&& (situacaoEspecialCobrancaHelper.getIndicadorResidencial() == null
								|| situacaoEspecialCobrancaHelper.getIndicadorResidencial().toString()
										.equals("" + ConstantesSistema.NAO))))) {
		...
		} else if (!valor.equals("COM") && ((situacaoEspecialCobrancaHelper.getIndicadorComercial() == null
				|| situacaoEspecialCobrancaHelper.getIndicadorComercial().toString().equals("" + ConstantesSistema.NAO))
				&& (situacaoEspecialCobrancaHelper.getIndicadorIndustrial() == null || situacaoEspecialCobrancaHelper
						.getIndicadorIndustrial().toString().equals("" + ConstantesSistema.NAO))
				&& (situacaoEspecialCobrancaHelper.getIndicadorPublico() == null || situacaoEspecialCobrancaHelper
						.getIndicadorPublico().toString().equals("" + ConstantesSistema.NAO))
				&& (situacaoEspecialCobrancaHelper.getIndicadorResidencial() == null || situacaoEspecialCobrancaHelper
						.getIndicadorResidencial().toString().equals("" + ConstantesSistema.NAO)))) {
			...
			consulta = "select imovel.id, imovel.ultimaAlteracao "
				...
				&& !situacaoEspecialCobrancaHelper.getIndicadorComercial().toString()
						.equals("" + ConstantesSistema.NAO))
				|| (situacaoEspecialCobrancaHelper.getIndicadorIndustrial() != null && !situacaoEspecialCobrancaHelper
						.getIndicadorIndustrial().toString().equals("" + ConstantesSistema.NAO))
				|| (situacaoEspecialCobrancaHelper.getIndicadorPublico() != null && !situacaoEspecialCobrancaHelper
			...
			if (situacaoEspecialCobrancaHelper.getIndicadorComercial() != null && situacaoEspecialCobrancaHelper
					.getIndicadorComercial().toString().equals("" + ConstantesSistema.SIM)) {
				consulta = consulta + " " + Categoria.COMERCIAL;
				aux = true;
			}
			if (situacaoEspecialCobrancaHelper.getIndicadorIndustrial() != null && situacaoEspecialCobrancaHelper
					.getIndicadorIndustrial().toString().equals("" + ConstantesSistema.SIM) && aux) {
				consulta = consulta + ", " + Categoria.INDUSTRIAL;

			} else if (situacaoEspecialCobrancaHelper.getIndicadorIndustrial() != null && situacaoEspecialCobrancaHelper
					.getIndicadorIndustrial().toString().equals("" + ConstantesSistema.SIM) && !aux) {
				consulta = consulta + " " + Categoria.INDUSTRIAL;
				aux = true;
			}
			if (situacaoEspecialCobrancaHelper.getIndicadorPublico() != null && situacaoEspecialCobrancaHelper
					.getIndicadorPublico().toString().equals("" + ConstantesSistema.SIM) && aux) {

				consulta = consulta + ", " + Categoria.PUBLICO;

			} else if (situacaoEspecialCobrancaHelper.getIndicadorPublico() != null && situacaoEspecialCobrancaHelper
					.getIndicadorPublico().toString().equals("" + ConstantesSistema.SIM) && !aux) {
			...
			if (situacaoEspecialCobrancaHelper.getIndicadorResidencial() != null && situacaoEspecialCobrancaHelper
					.getIndicadorResidencial().toString().equals("" + ConstantesSistema.SIM) && aux) {
				consulta = consulta + ", " + Categoria.RESIDENCIAL;
	...
	/**
	 * [UC0457] - Encerrar Ordem de Servi�o. [SB0009] - Verificar Situa��o Especial
	 * de Faturamento.
	 * 
	 * Verifica se um im�vel est� em situa��o especial de faturamento para um dado
	 * imovel (idImovel). A situa��o especial de faturamento tem o ftst_id = 2
	 * 
	 * @param idImovel
	 * @return Imovel
	 * @throws ErroRepositorioException
	...
	public Imovel pesquisarImovelSituacaoEspecialFaturamento(Integer idImovel) throws ErroRepositorioException {
		...
		try {
			...
			List<Imovel> imoveis = (List<Imovel>) session.createQuery(sb.toString()).setInteger("idImovel", idImovel)
					.list();
	...
	public Integer validarMesAnoReferencia(
		...
		if (helper != null
			...
			if (helper.getLocalidadeOrigemID() != null && !helper.getLocalidadeOrigemID().equals("")
				...
				consulta += " lo.id between " + helper.getLocalidadeOrigemID() + " and "
						+ helper.getLocalidadeDestinoID() + " and ";
			...
			if (helper.getSetorComercialOrigemID() != null && !helper.getSetorComercialOrigemID().equalsIgnoreCase("")
				...
				consulta += "sc.id between " + helper.getSetorComercialOrigemID() + " and "
						+ helper.getSetorComercialDestinoID() + " and ";

			if (helper.getCodigoRotaInicial() != null && !helper.getCodigoRotaInicial().equalsIgnoreCase("")
					&& !helper.getCodigoRotaInicial().equalsIgnoreCase(""))
				consulta += "rt.codigo between " + helper.getCodigoRotaInicial() + " and " + helper.getCodigoRotaFinal()
						+ " and ";

			if (!helper.getQuadraOrigemID().equalsIgnoreCase("") && !helper.getQuadraOrigemID().equalsIgnoreCase(""))
				consulta += "qu.id between " + helper.getQuadraOrigemID() + " and " + helper.getQuadraDestinoID()
						+ " and ";

			if (!helper.getLoteOrigem().equalsIgnoreCase("") && !helper.getLoteOrigem().equalsIgnoreCase(""))
				consulta += "im.lote between " + new Integer(helper.getLoteOrigem()) + " and "
						+ new Integer(helper.getLoteDestino()) + " and ";

			if (!helper.getSubloteOrigem().equalsIgnoreCase("") && !helper.getSubloteOrigem().equalsIgnoreCase(""))
				consulta += "im.subLote between " + new Integer(helper.getSubloteOrigem()) + " and "
						+ new Integer(helper.getSubloteDestino()) + " and ";
	...
	public void atualizarFaturamentoSituacaoTipo(Collection colecaoIdsImoveis, Integer idFaturamentoTipo)
		...
		try {
			...
			if (colecaoIdsImoveis != null && !colecaoIdsImoveis.isEmpty()) {
				logger.info("Atualizando " + colecaoIdsImoveis.size() + " Imoveis para o idFaturamentoTipo = "
						+ idFaturamentoTipo);
				...
				if (colecaoIdsImoveis.size() > 999) {
					...
					session.createQuery(consulta).setInteger("idFaturamentoSituacao", idFaturamentoTipo.intValue())
							.setParameterList("ids", colecaoIdsImoveis).setTimestamp("ultimaAlteracao", new Date())
							.executeUpdate();
				}
	...
	public void retirarSituacaoEspecialFaturamento(Collection colecaoIdsImoveis) throws ErroRepositorioException {
		...
		try {
			if (colecaoIdsImoveis != null && !colecaoIdsImoveis.isEmpty()) {
				...
				session.createQuery(consulta).setParameterList("ids", colecaoIdsImoveis)
	 ...
	 * 
	 * 
	 * [UC0164]Gerar Relat�rio de Im�vel Outros Crit�rios
	 * 
	 * 
	 * 
	 * Monta a Condi��o do where do Filtrar Imoveis Outros Criterios
	 * 
	 * 
	 * 
	...
	public void informarDadosQueryFiltrarImovelOutrosCriterio(SQLQuery query,
			...
			String sequencialRotaFinal, String[] pocoTipoListIds, String cpfCnpj) {
		...
		/*
		 * Alterado por S�vio Luiz Data: 14/09/2007 (Analista:Rosana) Inserir os
		 * parametros de c�digo de rota e sequencial de rota
		 */

		// c�digo de rota inicial e final
		...
		if ((sequencialRotaInicial != null && !sequencialRotaInicial.equals("") && !sequencialRotaInicial
				...
				&& (sequencialRotaFinal != null && !sequencialRotaFinal.equals("") && !sequencialRotaFinal
			...
			query.setInteger("sequencialRotaInicial", new Integer(sequencialRotaInicial)
	...
	/**
	 * [UC0164] Filtrar Imoveis por Outros Criterios Filtra para saber a quantidade
	 * de imoveis antes de executar o filtro
	 *
	 * @author Rafael Santos
	 * @date 01/08/2006
	 */
	...
	public Integer obterQuantidadaeRelacaoImoveisDebitos(String idImovelCondominio, String idImovelPrincipal,
		...
		try {
			...
			consulta = consulta + montarInnerJoinFiltrarImoveisOutrosCriterios(intervaloMediaMinimaImovelInicial,
					...
					idSituacaoEspecialCobranca, idEloAnormalidade, idCadastroOcorrencia, idConsumoTarifa, idTipoMedicao,
					indicadorMedicao, idSubCategoria, idCategoria, idCliente, idClienteTipo, idClienteRelacaoTipo,
					relatorio, null);

			consulta = consulta + montarCondicaoSQLWhereFiltrarImovelOutrosCriterio(idImovelCondominio,
					...
					idAreaConstruidaFaixa, idUnidadeNegocio, relatorio, cdRotaInicial, cdRotaFinal,
					sequencialRotaInicial, sequencialRotaFinal, null, null, null);
			...
			informarDadosQueryFiltrarImovelOutrosCriterio(query, idImovelCondominio, idImovelPrincipal,
					...
					cdRotaInicial, cdRotaFinal, sequencialRotaInicial, sequencialRotaFinal, null, null);

			retorno = (Integer) query.addScalar("quantidade", Hibernate.INTEGER).uniqueResult();
	...
	private String montarCondicaoSQLWhereFiltrarImovelOutrosCriterio(String idImovelCondominio,
			...
			String intervaloMediaMinimaHidrometroFinal, String idImovelPerfil, String idPocoTipo,
			String idFaturamentoSituacaoTipo, String idCobrancaSituacaoTipo, String idSituacaoEspecialCobranca,
			String idEloAnormalidade, String areaConstruidaInicial, String areaConstruidaFinal,
			...
			String numeroMoradoresInicial, String numeroMoradoresFinal, String idAreaConstruidaFaixa,
			String idUnidadeNegocio, Integer relatorio, String cdRotaInicial, String cdRotaFinal,
			String sequencialRotaInicial, String sequencialRotaFinal, String[] pocoTipoListIds, String indicadorCpfCnpj,
			String cpfCnpj) {

		String consulta = "";

		consulta = consulta + " where imovel.imov_icexclusao != " + Imovel.IMOVEL_EXCLUIDO + " and  ";
		// cliente
		if (idCliente != null && !idCliente.equals("")
		...
		// cliente tipo
		if (idClienteTipo != null && !idClienteTipo.equals("") && !idClienteTipo.trim()
				.equalsIgnoreCase(new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO).toString())) {
		...
		if (idClienteRelacaoTipo != null && !idClienteRelacaoTipo.equals("") && !idClienteRelacaoTipo.trim()
				.equalsIgnoreCase(new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO).toString())) {
		...
		// gerencia regional
		if (idGerenciaRegional != null && !idGerenciaRegional.equals("") && !idGerenciaRegional.trim()
				.equalsIgnoreCase(new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO).toString())) {
		...
		if (((setorComercialInicial != null && !setorComercialInicial.equals("")
				&& !setorComercialInicial.trim()
						.equalsIgnoreCase(new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO).toString()))
				&& (setorComercialFinal != null && !setorComercialFinal.equals("") && !setorComercialFinal.trim()
						.equalsIgnoreCase(new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO).toString())))) {
		...
		if ((quadraInicial != null && !quadraInicial.equals("")
				&& !quadraInicial.trim()
						.equalsIgnoreCase(new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO).toString()))
				&& (quadraFinal != null && !quadraFinal.equals("") && !quadraFinal.trim()
						.equalsIgnoreCase(new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO).toString()))) {
		...
		if (((cdRotaInicial != null && !cdRotaInicial.equals("")
				&& !cdRotaInicial.trim()
						.equalsIgnoreCase(new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO).toString()))
				&& (cdRotaFinal != null && !cdRotaFinal.equals("") && !cdRotaFinal.trim()
						.equalsIgnoreCase(new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO).toString())))) {
		...
		if (((sequencialRotaInicial != null && !sequencialRotaInicial.equals("")
				&& !sequencialRotaInicial.trim()
						.equalsIgnoreCase(new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO).toString()))
				&& (sequencialRotaFinal != null && !sequencialRotaFinal.equals("") && !sequencialRotaFinal.trim()
						.equalsIgnoreCase(new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO).toString())))) {
		...
		if ((loteOrigem != null && !loteOrigem.equals("")
				&& !loteOrigem.trim().equalsIgnoreCase(new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO).toString()))
				&& (loteDestno != null && !loteDestno.equals("") && !loteDestno.trim()
						.equalsIgnoreCase(new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO).toString()))) {
		...
		// logradouro
		if (logradouro != null && !logradouro.equals("") && !logradouro.trim()
				.equalsIgnoreCase(new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO).toString())) {
		...
		if ((consumoMinimoInicialAgua != null && !consumoMinimoInicialAgua.equals("")
				&& !consumoMinimoInicialAgua.trim()
						.equalsIgnoreCase(new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO).toString()))
				&& (consumoMinimoFinalAgua != null && !consumoMinimoFinalAgua.equals("") && !consumoMinimoFinalAgua
		...
		// imovel condominio
		if (idImovelCondominio != null && !idImovelCondominio.equals("") && !idImovelCondominio.trim()
				.equalsIgnoreCase(new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO).toString())) {
		...
		// imovel principal
		if (idImovelPrincipal != null && !idImovelPrincipal.equals("") && !idImovelPrincipal.trim()
				.equalsIgnoreCase(new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO).toString())) {
		...
		// situa��o da liga��o de agua
		if (idSituacaoLigacaoAgua != null && !idSituacaoLigacaoAgua.equals("") && !idSituacaoLigacaoAgua.trim()
				.equalsIgnoreCase(new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO).toString())) {
		...
		// situa��o liga��o de esgoto
		if (idSituacaoLigacaoEsgoto != null && !idSituacaoLigacaoEsgoto.equals("") && !idSituacaoLigacaoEsgoto.trim()
				.equalsIgnoreCase(new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO).toString())) {
		...
		// imovel Perfil
		if (idImovelPerfil != null && !idImovelPerfil.equals("") && !idImovelPerfil.trim()
				.equalsIgnoreCase(new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO).toString())) {
		...
		// po�o tipo
		if (idPocoTipo != null && !idPocoTipo.equals("") && !idPocoTipo.trim()
				.equalsIgnoreCase(new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO).toString())) {
		...
		// faturamento situacao tipo
		if (idFaturamentoSituacaoTipo != null && !idFaturamentoSituacaoTipo.equals("") && !idFaturamentoSituacaoTipo
				.trim().equalsIgnoreCase(new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO).toString())) {
		...
		// Situacao Especial Cobranca
		if (idSituacaoEspecialCobranca != null && !idSituacaoEspecialCobranca.equals("") && !idSituacaoEspecialCobranca
				.trim().equalsIgnoreCase(new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO).toString())) {
		...
		// cobranca situacao tipo
		if (idCobrancaSituacaoTipo != null && !idCobrancaSituacaoTipo.equals("") && !idCobrancaSituacaoTipo.trim()
				.equalsIgnoreCase(new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO).toString())) {
		...
		// elo anormalidade
		if (idEloAnormalidade != null && !idEloAnormalidade.equals("") && !idEloAnormalidade.trim()
				.equalsIgnoreCase(new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO).toString())) {
		...
		// cadastro ocorrencia
		if (idCadastroOcorrencia != null && !idCadastroOcorrencia.equals("") && !idCadastroOcorrencia.trim()
				.equalsIgnoreCase(new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO).toString())) {
		...
		// consumo tarifa
		if (idConsumoTarifa != null && !idConsumoTarifa.equals("") && !idConsumoTarifa.trim()
				.equalsIgnoreCase(new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO).toString())) {
		...
		// categoria
		if (idCategoria != null && !idCategoria.equals("") && !idCategoria.trim()
				.equalsIgnoreCase(new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO).toString())) {
		...
		// sub categoria
		if (idSubCategoria != null && !idSubCategoria.equals("") && !idSubCategoria.trim()
				.equalsIgnoreCase(new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO).toString())) {
		...
		if (relatorio != null
			...
			if ((numeroPontosInicial != null && !numeroPontosInicial.equals("")
					&& !numeroPontosInicial.trim()
							.equalsIgnoreCase(new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO).toString()))
					&& (numeroPontosFinal != null && !numeroPontosFinal.equals("") && !numeroPontosFinal.trim()
							.equalsIgnoreCase(new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO).toString()))) {
			...
			if (idAreaConstruidaFaixa != null && !idAreaConstruidaFaixa.equals("") && !idAreaConstruidaFaixa.trim()
					.equalsIgnoreCase(new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO).toString())) {
			...
			if ((areaConstruidaInicial != null && !areaConstruidaInicial.equals("")
					&& !areaConstruidaInicial.trim()
							.equalsIgnoreCase(new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO).toString()))

					&& (areaConstruidaFinal != null && !areaConstruidaFinal.equals("") && !areaConstruidaFinal.trim()
							.equalsIgnoreCase(new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO).toString()))) {
		...
		// dia Vencimento
		if (diaVencimento != null && !diaVencimento.equals("") && !diaVencimento.trim()
				.equalsIgnoreCase(new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO).toString())) {
		...
		// po�o tipo lista
		if (pocoTipoListIds != null && (((pocoTipoListIds[0].equals("-1") && pocoTipoListIds.length > 1))
				|| (!pocoTipoListIds[0].equals("-1")))) {
		...
		if (indicadorCpfCnpj != null && indicadorCpfCnpj.equals("2")) {
			consulta = consulta + " cliente_usuario.clie_nncpf is null and cliente_usuario.clie_nncnpj is null and  ";
		...
		if (cpfCnpj != null && !cpfCnpj.equals("")) {
			...
			if (idClienteRelacaoTipo != null && idClienteRelacaoTipo.trim()
					.equalsIgnoreCase(new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO).toString())) {
				consulta = consulta + " cliente_imovel_usuario.crtp_id = " + ClienteRelacaoTipo.USUARIO + " and  ";
			}
	...
	public Collection gerarRelatorioImovelOutrosCriterios(String idImovelCondominio, String idImovelPrincipal,
		...
		try {
			...
			consulta = consulta + "select distinct " + "gerencia_regional.greg_id, "
					+ "gerencia_regional.greg_nmabreviado, " + "localidade.loca_id, " + "localidade.loca_nmlocalidade, "
					+ "imovel.imov_id, " + "imovel.imov_qteconomia, " + "setor_comercial.stcm_cdsetorcomercial, "
					+ "setor_comercial.stcm_nmsetorcomercial, " + "quadra.qdra_nnquadra, " + "imovel.imov_nnlote, "
					+ "imovel.imov_nnsublote, " + "ligacao_agua_situacao.last_dsabreviado, "
					+ "ligacao_esgoto_situacao.lest_dsabreviado, " + "ligacao_esgoto.lesg_pcesgoto, "
					+ "ligacao_esgoto.lesg_dtligacao, " + "ligacao_agua.lagu_dtligacaoagua, "
					// Informa��es do Cliente Usuasrio e Resposanvel
					...
					+ "logradouro.logr_nmlogradouro, " + "logradouro_tipo.lgtp_dslogradourotipo, "
					+ "logradouro_titulo.lgtt_dslogradourotitulo, " + "cep.cep_cdcep, "
					+ "endereco_referencia.edrf_dsenderecoreferencia, " + "imovel.imov_dscomplementoendereco, "
					+ "imovel.imov_nnimovel, " + "bairro.bair_nmbairro, " + "municipio.muni_nmmunicipio, "
					+ "unidade_federacao.unfe_dsufsigla, " + "imovel.imov_icimovelcondominio, "
					+ "imovel.imov_nnmorador, " + "imovel.imov_idimovelcondominio, " + "imovel.imov_idimovelprincipal, "
					+ "imovel.imov_nnpontosutilizacao, " + "imovel_perfil.iper_dsimovelperfil, "
					+ "area_construida_faixa.acon_nnmaiorfaixa, " + "area_construida_faixa.acon_nnmenorfaixa, "
					+ "imovel.imov_nnareaconstruida, " + "pavimento_calcada.pcal_dspavimentocalcada, "
					+ "pavimento_rua.prua_dspavimentorua, " + "despejo.depj_dsdespejo, "
					+ "reservatorio_volume_fx.resv_vomenorfaixa, " + "reservatorio_volume_fx.resv_vomaiorfaixa, "
					+ "imovel.imov_voreservatoriosuperior, " + "reservatorio_volume_fx_infe.resv_vomenorfaixa, "
					+ "reservatorio_volume_fx_infe.resv_vomaiorfaixa, " + "imovel.imov_voreservatorioinferior, "
					+ "piscina_volume_faixa.pisc_vomenorfaixa, " + "piscina_volume_faixa.pisc_vomaiorfaixa, "
					+ "imovel.imov_vopiscina, " + "poco_tipo.poco_dspocotipo, "
					...
					+ "ligacao_esgoto.lesg_nnconsumominimoesgoto, " + "ligacao_esgoto.lesg_pccoleta, "
...
//					+ "consumo_historico.cshi_nnconsumomedio as consumoMedio "
			...
			consulta = consulta + montarCondicaoSQLWhereFiltrarImovelOutrosCriterio(idImovelCondominio,
					...
					quantidadeEconomiasFinal, diaVencimento, idCliente, idClienteTipo, idClienteRelacaoTipo,
					numeroPontosInicial, numeroPontosFinal, numeroMoradoresInicial, numeroMoradoresFinal,
					idAreaConstruidaFaixa, idUnidadeNegocio, ConstantesSistema.GERAR_RELATORIO_IMOVEL, cdRotaInicial,
					cdRotaFinal, sequencialRotaInicial, sequencialRotaFinal, null, null, null);
...
//			consulta = consulta + " order by loca_id, stcm_cdsetorcomercial, qdra_nnquadra, imov_nnlote, imov_nnsublote ";
			...
			if (sistemaParametro.getNomeAbreviadoEmpresa() != null
					&& sistemaParametro.getNomeAbreviadoEmpresa().equals("COMPESA")) {
				consulta = consulta
						+ " ORDER BY loca_id, stcm_cdsetorcomercial, qdra_nnquadra, imov_nnlote, imov_nnsublote ";

			} else {
				consulta = consulta + " ORDER BY imov_nnsequencialrota, imov_nnlote, imov_nnsublote ";
			}
			...
			retorno = query.addScalar("greg_id", Hibernate.INTEGER).addScalar("greg_nmabreviado", Hibernate.STRING)
					...
					.addScalar("stcm_nmsetorcomercial", Hibernate.STRING).addScalar("qdra_nnquadra", Hibernate.INTEGER)
					.addScalar("imov_nnlote", Hibernate.SHORT).addScalar("imov_nnsublote", Hibernate.SHORT)
					.addScalar("last_dsabreviado", Hibernate.STRING).addScalar("lest_dsabreviado", Hibernate.STRING)
					.addScalar("lesg_pcesgoto", Hibernate.BIG_DECIMAL).addScalar("lesg_dtligacao", Hibernate.DATE)
					.addScalar("lagu_dtligacaoagua", Hibernate.DATE).addScalar("idClienteUsuario", Hibernate.INTEGER)
					...
					.addScalar("imov_nnimovel", Hibernate.STRING).addScalar("bair_nmbairro", Hibernate.STRING)
					.addScalar("muni_nmmunicipio", Hibernate.STRING).addScalar("unfe_dsufsigla", Hibernate.STRING)
					...
					.addScalar("acon_nnmaiorfaixa", Hibernate.INTEGER).addScalar("acon_nnmenorfaixa", Hibernate.INTEGER)
					.addScalar("imov_nnareaconstruida", Hibernate.BIG_DECIMAL)
					.addScalar("pcal_dspavimentocalcada", Hibernate.STRING)
					.addScalar("prua_dspavimentorua", Hibernate.STRING).addScalar("depj_dsdespejo", Hibernate.STRING)
					...
					.addScalar("lesg_pccoleta", Hibernate.BIG_DECIMAL).addScalar("lesg_pcesgoto", Hibernate.BIG_DECIMAL)
					...
					.addScalar("lesg_nnconsumominimoesgoto", Hibernate.INTEGER)
					.addScalar("imov_icjardim", Hibernate.SHORT).addScalar("rota_cdrota", Hibernate.SHORT)
					.addScalar("imov_nnsequencialrota", Hibernate.INTEGER).addScalar("logr_id", Hibernate.INTEGER)
//					.addScalar("consumoMedio", Hibernate.INTEGER)
		...
		try {
			...
			Collection colecaoConsulta = session.createSQLQuery(sql).addScalar(
					...
					.addScalar(
							...
							"numeroQuadra", Hibernate.INTEGER)
					.addScalar("lote",
	...
	/**
	 * O m�todo abaixo realiza uma pesquisa em imovel e retorna os campos
	 * necess�rios para a cria��o da inscri��o para exibi��o, independente do im�vel
	 * ter sido exclu�do ou n�o.
	 * 
	 * acima no controlador ser� montada a inscri��o
	 */
	public Object[] pesquisarInscricaoImovelExcluidoOuNao(Integer idImovel) throws ErroRepositorioException {
		...
		try {
			...
			String sql = "select loc.loca_id as idLocalidade, " + " sc.stcm_cdsetorcomercial as codigoSetorComercial, "
					+ "	qd.qdra_nnquadra as numeroQuadra, " + "	im.imov_nnlote as lote, "
					+ "	im.imov_nnsublote as subLote " + " from cadastro.imovel im, " + "	cadastro.localidade loc, "
					+ "	cadastro.setor_comercial sc, " + "	cadastro.quadra qd "
					+ " where loc.loca_id = im.loca_id and " + "	im.stcm_id = sc.stcm_id and "
					+ "	im.qdra_id = qd.qdra_id and " + "	im.imov_id = " + idImovel;

			Collection colecaoConsulta = session.createSQLQuery(sql).addScalar("idLocalidade", Hibernate.INTEGER)
					.addScalar("codigoSetorComercial", Hibernate.INTEGER).addScalar("numeroQuadra", Hibernate.INTEGER)
					.addScalar("lote", Hibernate.SHORT).addScalar("subLote", Hibernate.SHORT).list();
		...
		try {
			...
			consulta = consulta
					...
					+ montarCondicaoSQLWhereFiltrarImovelOutrosCriterio(
							...
							ConstantesSistema.GERAR_RELATORIO_DADOS_ECONOMIA_IMOVEL, null, null, null, null, null, null,
							null);
		...
		try {
			...
			retorno = query.addScalar("scat_dssubcategoria", Hibernate.STRING)
					...
					.addScalar("catg_dscategoria", Hibernate.STRING).addScalar(
							...
							"imsb_qteconomia", Hibernate.SHORT)
					.addScalar(
		...
		try {
			...
			retorno = query.addScalar("imec_dscomplementoendereco",
					...
					Hibernate.STRING).addScalar("imec_nnpontosutilizacao",
							...
							Hibernate.SHORT)
					.addScalar("imec_nnmorador",

							Hibernate.SHORT)
					.addScalar("imec_nniptu",

							Hibernate.BIG_DECIMAL)
					.addScalar("imec_nncontratoenergia",

							Hibernate.LONG)
					.addScalar("imec_nnareaconstruida",
		...
		try {
			...
			retorno = query.addScalar("clie_nmcliente", Hibernate.STRING)
					...
					.addScalar("clie_nncpf", Hibernate.STRING).addScalar(
							...
							"clie_nncnpj", Hibernate.STRING)
					.addScalar(

							"cime_dtrelacaoinicio", Hibernate.DATE)
					.addScalar(

							"cime_dtrelacaofim", Hibernate.DATE)
					.addScalar(

							"cifr_dsfimrelacaomotivo", Hibernate.STRING)
					.list();
	...
	public String montarInnerJoinFiltrarImoveisOutrosCriterios(String intervaloMediaMinimaImovelInicial,
			String intervaloMediaMinimaImovelFinal, String intervaloMediaMinimaHidrometroInicial,
			String intervaloMediaMinimaHidrometroFinal, String idFaturamentoSituacaoTipo, String idCobrancaSituacaoTipo,
			String idSituacaoEspecialCobranca, String idEloAnormalidade, String idCadastroOcorrencia,
			String idConsumoTarifa, String idTipoMedicao, String indicadorMedicao, String idSubCategoria,
			String idCategoria, String idCliente, String idClienteTipo, String idClienteRelacaoTipo, Integer relatorio,
			String cpfCnpj) {
		...
		if (((idClienteRelacaoTipo != null && !idClienteRelacaoTipo.equals("")
				&& !idClienteRelacaoTipo.trim()
						.equalsIgnoreCase(new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO).toString())))
				|| ((idCliente != null && !idCliente.equals("") && !idCliente.trim()
						.equalsIgnoreCase(new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO).toString())))) {
		...
		if (idClienteTipo != null && !idClienteTipo.equals("") && !idClienteTipo.trim()
				.equalsIgnoreCase(new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO).toString())) {
			if (!(idCliente != null && !idCliente.equals("") && !idCliente.trim()
					.equalsIgnoreCase(new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO).toString()))) {
	...
	public Collection<EntidadeBeneficente> pesquisarEntidadeBeneficente(
		...
		try {
			...
			/**
			 * * executa o m�todo para pesquisa de pesquisa de Entidade
			 * 
			 * Beneficente **
			 * 
			 */
	...
	public Collection<ImovelDoacao> pesquisarImovelDoacao(
		...
		try {
			...
			/** * executa o m�todo para pesquisa de pesquisa de Imovel Doa��o ** */
	...
	public void atualizarImovelDoacao(ImovelDoacao imovelDoacao)
		...
		try {
			...
			/** * Atualiza as informa��es do im�vel doa��o ** */
	...
	public Collection consultarImovelDadosCadastrais(Integer idImovel)
		...
		try {
			...
			consulta = "SELECT ligacaoAguaSituacao.descricao, "
					...
					+ "left join imovel.imovelTipoCobertura tipoCobertura " + "left join imovel.localidade localidade "
					+ "left join localidade.municipio municipio " + "where imovel.id = :idImovel ";
	...
	/**
	 * Pesquisa a cole��o de clientes do imovel [UC0472] Consultar Imovel
	 * 
	...
	public Collection pesquisarClientesImovel(Integer idImovel) throws ErroRepositorioException {
		...
		try {
			...
			consulta = "SELECT cliente.id," + // 0
					...
					"clienteImovel.dataInicioRelacao," + // 3
					"clienteFone.telefone," + // 4
					"cliente.cnpj," + // 5
					"cliente.cpf, " + // 6
					"clienteFone.ddd, " + // 7
					"clienteRelacaoTipo.id, " + // 8
					"clienteImovel.id, " + // 9
					"cliente.rg, " + // 10
					"cliente.indicadorUso " + // 11
	...
	public Collection pesquisarClientesImovelDataMax(Integer idImovel) throws ErroRepositorioException {
		...
		try {
			...
			retorno = session.createQuery(consulta).setInteger("idImovel", idImovel.intValue())
					.setInteger("indicadorExclusao", ConstantesSistema.SIM).list();
	...
	/**
	 * Pesquisa a cole��o de clientes do imovel mesmo que o im�vel j� tenha sido
	 * exclu�do [UC0472] Consultar Imovel
	 * 
	 * @param filtroClienteImovel parametros para a consulta
	 * @return Description of the Return Value
	 * @exception ErroRepositorioException Description of the Exception
	...
	public Collection consultarImovelDadosComplementares(Integer idImovel) throws ErroRepositorioException {
		...
		try {
			...
			consulta = "SELECT ligacaoAguaSituacao.descricao, " + // 0
					...
					+ "left join imovel.ligacaoAguaSituacao ligacaoAguaSituacao "
					+ "left join imovel.ligacaoEsgotoSituacao ligacaoEsgotoSituacao "
					+ "left join imovel.funcionario funcionario " + "where imovel.id = :idImovel ";
	...
	public Collection pesquisarFaturamentosSituacaoHistorico(Integer idImovel)
		...
		try {
			...
			consulta = "SELECT faturamentoSituacaoTipo.id,"
					...
					"case when (usuario  is null ) then " + "  (case when (usuarioRetirada  is not null ) then"
					+ "   usuarioRetirada.nomeUsuario " + "  else " + "   usuarioInforma.nomeUsuario " + "  end ) "
					+ "else " + "usuario.nomeUsuario " + "end, " + // 7
	...
	public Collection consultarImovelAnaliseMedicaoConsumo(Integer idImovel)
		...
		try {
			...
			// 0
			consulta = "SELECT ligacaoAguaSituacao.descricao, " + // 1
					...
					+ // 12
					" ligacaoAgua.numeroSeloSupressao, " + // 13
					" imovel.dataSupressaoParcial, " + // 14
	...
	public Cliente consultarClienteImovel(Imovel imovel, Short clienteRelacaoTipo)
		...
		try {
			...
			consulta = "SELECT cli " + "from ClienteImovel cliimo " + "left join cliimo.cliente cli "
					+ "left join fetch cli.clienteFones cliFones " + "left join fetch cli.clienteTipo cliTipo "
			...
			resultadoConsultar = (Cliente) session.createQuery(consulta).setInteger("idImovel", imovel.getId())
					.setShort("tipoRelacao", clienteRelacaoTipo).setMaxResults(1).uniqueResult();
	...
	public String consultarNomeClienteImovel(Integer idImovel, Short relacaoTipo)
		...
		try {
			...
			consulta.append("SELECT cliente.nome ").append("from ClienteImovel clienteImovel ")
			...
			resultadoConsultar = session.createQuery(consulta.toString()).setInteger("idImovel", idImovel.intValue())
					.setShort("relacaoTipo", relacaoTipo).uniqueResult();
	...
	// Altera��o para n�o carregar contas na situa��o ERRO_PROCESSAMENTO
	...
	public Collection pesquisarClienteUsuarioImovelExcluidoOuNao(Integer idImovel) throws ErroRepositorioException {
		...
		try {
			...
			retorno = (Collection) session.createQuery(consulta).setInteger("idImovel", idImovel.intValue())
					.setShort("idClienteUsuario", ClienteRelacaoTipo.USUARIO.shortValue()).setMaxResults(1).list();
		...
		try {
			...
			session.createQuery(update).setInteger("codigoImovel",
					...
					codigoImovel.intValue()).setInteger("numeroParcelamento",
							...
							numeroParcelamento.intValue())
					.setInteger(

							"numeroReparcelamentoConsecutivo",

							numeroReparcelamentoConsecutivo.intValue())
					.setInteger(
	...
	public Collection<Categoria> pesquisarCategoriasImovel(Imovel imovel)
		...
		// pesquisa a cole��o de categorias e atribui a vari�vel "retorno"
		...
		try {
			...
			consulta = "select distinct logradouro.logr_nmlogradouro as nomeLogradouro, " // + " logradouro.nome," + //
					...
					+ " logradouroTipo.lgtp_dsabreviado as logradouroTipoAbreviado, " // "logradouroTipo.descricaoAbreviada,"
																						// // 1
					+ " logradouroTitulo.lgtt_dsabreviado as logradouroTituloAbreviado, "// "
																							// logradouroTitulo.descricaoAbreviada,"
																							// // 2
			...
			consulta = consulta + " imovel.imov_nnlote as lote, " // "imovel.lote, " // 27
					...
					+ "left join cadastro.logradouro_cep logradouroCep " + "on logradouroCep.lgcp_id = imovel.lgcp_id "

					+ "left join cadastro.cep cep " + "on cep.cep_id = logradouroCep.cep_id "
					...
					+ "left join cadastro.bairro bairro " + "on bairro.bair_id = logradouroBairro.bair_id "

					+ "left join cadastro.municipio municipio " + "on municipio.muni_id = bairro.muni_id "
			...
			if (pesquisarImovelManterVinculo) {
				...
				consulta = consulta + " left outer join cadastro.cliente_imovel clienteImovelUsuario "
						+ " on imovel.imov_id=clienteImovelUsuario.imov_id and " + " clienteImovelUsuario.crtp_id = "
						+ ClienteRelacaoTipo.USUARIO.toString() + " and clienteImovelUsuario.clim_dtrelacaofim is null "
...
//						+ " left outer join clienteImoveisUsuario.cliente cliente ";
				...
				consulta = consulta + " left outer join cadastro.cliente cliente "
...
//				+ " left join clienteImovel.cliente cliente ";
			...
			if (numeroImovelInicial != null && !numeroImovelInicial.trim().equals("")) {
				...
				consulta = consulta + "RTRIM(LTRIM(translate(imovel.imov_nnimovel, '"
						+ ConstantesSistema.CARACTERES_ALFANUMERICOS + "', ''))) <> '' and "
						+ "to_number(RTRIM(LTRIM(translate(imovel.imov_nnimovel, '"
						+ ConstantesSistema.CARACTERES_ALFANUMERICOS + "', ''))), 99999) between '"
			...
			Query query = null;
			if (pesquisarImovelManterVinculo) {
				query = session.createSQLQuery(consulta).addScalar("nomeLogradouro", Hibernate.STRING) // 0
	...
	public Object pesquisarQuantidadeImovel(String idImovel,
		...
		try {
			...
			consulta = consulta + " left outer join cadastro.imovel imovel "
					...
					+ "left join cadastro.logradouro_bairro logradouroBairro "
					+ "on logradouroBairro.lgbr_id = imovel.lgbr_id " + "left join cadastro.bairro bairro "
			...
			if ((numeroQuadra != null && !numeroQuadra.equals("") && !numeroQuadra
				...
				consulta = consulta + "inner join cadastro.quadra quadra " + "on quadra.qdra_id = imovel.qdra_id ";
			...
			if (codigoCliente != null
				...
				if (pesquisarImovelManterVinculo) {
					...
					consulta = consulta
							...
							+ " on clienteImovelUsuario.crtp_id = " + ClienteRelacaoTipo.USUARIO.toString()
...
//					+ " left join clienteImovel.cliente cliente ";
			...
			/**
			 * autor: Adriana Muniz data: 17/09/2012 tratamento de exce��o, caso seja
			 * exce��o devido a convers�o de alfanumerico para numerico
			 */
			try {
	 ...
	 * 
	 * @alteracao: 26/02/2010 - CRC3755 - Adicionado a condicao de apenas clientes
	 *             com relacao do tipo USUARIO;
	 * 
	 * 
	 * 
	 * @param idImovel
...
//					+ " cep.codigo, " // 16
					...
					// CRC 3928 -- Retirado por R�mulo Aur�lio a pedido de Aryed Lins 05/03/2010
...
//			}
			...
			if (numeroImovelInicial != null && !numeroImovelInicial.trim().equals("") && numeroImovelFinal != null
					&& !numeroImovelFinal.trim().equals("")) {
				consulta += " RTRIM(LTRIM(translate(imovel.imov_nnimovel, '"
						+ ConstantesSistema.CARACTERES_ALFANUMERICOS + "', ''))) <> '' and ";

				consulta += " to_number(RTRIM(LTRIM(translate(imovel.imov_nnimovel, '"
						+ ConstantesSistema.CARACTERES_ALFANUMERICOS + "', ''))), 99999) between '"
						+ numeroImovelInicial.trim() + "' and '" + numeroImovelFinal.trim() + "' and ";
			}
			...
			Query query = session.createSQLQuery(consulta.substring(0, (consulta.length() - 5)))
	...
	public Collection pesquisarImoveisPorRotaComPaginacaoSemRotaAlternativa(Rota rota,
			Collection idsSituacaoLigacaoAgua, Collection idsSituacaoLigacaoEsgoto, Integer numeroInicial,
			int numeroMaximo) throws ErroRepositorioException {
		...
		try {
			...
			consulta = "select im.id," // 0
					...
					+ "where rt.id = :idRota " + "and im.indicadorExclusao = "
					+ ConstantesSistema.INDICADOR_IMOVEL_ATIVO;
			...
			retorno = (Collection) q.setFirstResult(numeroInicial).setMaxResults(numeroMaximo).list();
	...
	public Collection pesquisarImoveisPorRotaComPaginacaoComRotaAlternativa(Rota rota,
		...
		try {
			...
			consulta = "select im.id," // 0
					...
					+ "where im.rotaAlternativa.id = :idRota " + "and im.indicadorExclusao = "
					+ ConstantesSistema.INDICADOR_IMOVEL_ATIVO;
	 ...
	 * 
	 * 
	 * [UC0978] Associar Tarifa de Consumo a Im�veis
	 * 
	 * 
	 * 
	 ...
	 *                             quadraFinal,
	 * 
	 *                             loteInicial, loteFinal, subLoteInicial,
	 *                             subLoteFinal,
	 * 
	 *                             idTarifaAnterior
	 * 
	...
	public Integer verificarNumeroCompanhiaEletrica(
		...
		try {
			...
			if (idImovel != null) {
				...
				consulta = "SELECT imov.imov_id as idImovel " + " FROM cadastro.imovel imov " + " WHERE "
						+ " imov.imov_nncontratoenergia = :numeroCompanhiaEletrica " + " AND imov.imov_id <> :idImovel "
						+ " UNION " + " SELECT imovel.imov_id as idImovel " + " FROM cadastro.imovel_economia imovEcon "
						+ " INNER JOIN cadastro.imovel imovel " + " on imovEcon.imov_id = imovel.imov_id " + " WHERE "
				...
				consulta = " SELECT imovel.imov_id as idImovel " + " FROM cadastro.imovel_economia imovEcon "
						+ " INNER JOIN cadastro.imovel imovel " + " on imovEcon.imov_id = imovel.imov_id " + " WHERE "
						+ " imovEcon.imec_nncontratoenergia = :numeroCompanhiaEletrica "
						+ " AND imovEcon.imec_id <> :idImovelEconomia " + " UNION "
						+ " SELECT imov.imov_id as idImovel " + " FROM cadastro.imovel imov " + " WHERE "
	...
	public Imovel pesquisarImovel(Integer idImovel) throws ErroRepositorioException {
		...
		try {
			...
			consulta.append("SELECT imov FROM Imovel imov ")
		      ...
		      .append(" LEFT JOIN FETCH imov.faturamentoSituacaoTipo faturamentoSituacaoTipo")
	...
	public Collection<Imovel> pesquisarImoveis(Collection<Integer> idsImoveis) throws ErroRepositorioException {
		...
		try {
			consulta.append("SELECT imov FROM Imovel imov ").append(" LEFT JOIN FETCH imov.imovelPerfil imovelPerfil ")
					.append(" LEFT JOIN FETCH imov.localidade lo").append(" LEFT JOIN FETCH lo.gerenciaRegional geRe")
					.append(" LEFT JOIN FETCH imov.setorComercial sc").append(" LEFT JOIN FETCH imov.quadra qu")
					.append(" LEFT JOIN FETCH imov.quadraFace qdfa").append(" LEFT JOIN FETCH qu.rota rota")
					.append(" LEFT JOIN FETCH imov.logradouroBairro logBairro")
					.append(" LEFT JOIN FETCH logBairro.bairro bairro").append(" LEFT JOIN FETCH bairro.municipio")
					.append(" LEFT JOIN FETCH imov.ligacaoEsgoto lesg")
					.append(" LEFT JOIN FETCH imov.ligacaoEsgotoSituacao lest")
					.append(" LEFT JOIN FETCH imov.ligacaoAgua lagu")
					.append(" LEFT JOIN FETCH imov.ligacaoAguaSituacao last")
					...
					.append(" LEFT JOIN FETCH lagu.hidrometroInstalacaoHistorico hih")
					.append(" LEFT JOIN FETCH hih.hidrometroLocalInstalacao hil")
					.append(" LEFT JOIN FETCH hih.hidrometroProtecao hip").append(" LEFT JOIN FETCH hih.hidrometro hid")
					.append(" LEFT JOIN FETCH hid.hidrometroMarca him")
					.append(" LEFT JOIN FETCH hid.hidrometroCapacidade hic")
					.append(" LEFT JOIN FETCH imov.cadastroOcorrencia co").append(" WHERE  imov.id in (:idsImoveis)");

			imoveis = session.createQuery(consulta.toString()).setParameterList("idsImoveis", idsImoveis)
		...
		try {
			...
			session.createQuery(consulta).setInteger("idLogradouroCepNovo",
					...
					logradouroCepNovo.getId()).setTimestamp("ultimaAlteracao",
							...
							new Date())
					.setInteger("idLogradouroCepAntigo",

							logradouroCepAntigo.getId())
					.executeUpdate();
...
//	}
	...
	public Collection pesquisarColecaoImovel(
		 ...
		 * 
		 * 
		 * OBJETIVO: Acrescentar o par�metro grupo de faturamento para o filtro
		 * 
		 * de manuten��o de v�rias contas.
		 * 
		 */
		...
		/**
		 * @autor: Adriana Muniz
		 * @date: 24/11/2011 Acr�scimo de atributo esfera de poder na consulta
		 */
		try {
			 ...
			 * 
			 * 
			 * OBJETIVO: Acrescentar o par�metro grupo de faturamento para o
			 * 
			 * filtro de manuten��o de v�rias contas.
			 * 
			 */
	...
	public Integer pesquisarColecaoImovelPerfilBloqueado(FiltrarImovelInserirManterContaHelper filtro)
		...
		try {
			...
			consulta = "select count(imovel.id) " + "FROM gcom.cadastro.imovel.Imovel  imovel "
					+ "INNER JOIN imovel.quadra quadra " + "LEFT JOIN imovel.faturamentoSituacaoTipo ftst "
					+ "INNER JOIN quadra.rota rota " + "INNER JOIN rota.faturamentoGrupo ftgr "
					+ "INNER JOIN imovel.imovelPerfil iper " + "WHERE iper.indicadorBloquearRetificacao = 1 ";

			if (localidadeOrigemID != null && localidadeDestinoID != null) {
				consulta = consulta + "and imovel.localidade.id between " + localidadeOrigemID.intValue() + " and "
						+ localidadeDestinoID.intValue();
			}

			if (setorComercialOrigemID != null && setorComercialDestinoID != null) {
				consulta = consulta + " and imovel.setorComercial.id between " + setorComercialOrigemID.intValue()
						+ " and " + setorComercialDestinoID.intValue();
			}

			if (colecaoQuadraSelecionada != null && !colecaoQuadraSelecionada.isEmpty()) {
				consulta = consulta + " and imovel.quadra.id in (:colecaoQuadraSelecionada)";
			} else if (quadraOrigemID != null && quadraDestinoID != null) {
				consulta = consulta + " and imovel.quadra.id between " + quadraOrigemID.intValue() + " and "
						+ quadraDestinoID.intValue();
			}

			if (loteOrigem != 0 && loteDestino != 0) {
				consulta = consulta + " and imovel.lote between " + loteOrigem.intValue() + " and "
						+ loteDestino.intValue();
			}

			if (subLoteOrigem != 0 && subLoteDestino != 0) {
				consulta = consulta + " and imovel.subLote between " + subLoteOrigem.intValue() + " and "
						+ subLoteDestino.intValue();
			}

			if (codigoRotaOrigem != null && codigoRotaDestino != null) {
				consulta = consulta + " and rota.codigo between " + codigoRotaOrigem.shortValue() + " and "
						+ codigoRotaDestino.shortValue();
			}

			if (sequencialRotaOrigem != null && sequencialRotaDestino != null) {
				consulta = consulta + " and imovel.numeroSequencialRota between " + sequencialRotaOrigem.intValue()
						+ " and " + sequencialRotaDestino.intValue();
			}
			...
			if (colecaoQuadraSelecionada != null && !colecaoQuadraSelecionada.isEmpty()) {
				retorno = (Integer) session.createQuery(consulta)
						.setParameterList("colecaoQuadraSelecionada", colecaoQuadraSelecionada).uniqueResult();
			} else {
	...
	public Collection pesquisarColecaoImovelCliente(Integer codigoCliente, Integer relacaoTipo,
			Boolean verificarImovelPerfilBloqueado) throws ErroRepositorioException {
		...
		try {
			...
			consulta = "select distinct(imov.id) " + "from ClienteImovel clim " + "inner join clim.cliente clie "
					+ "inner join clim.imovel imov " + "inner join imov.imovelPerfil iper "
	...
	public Integer pesquisarColecaoImovelClienteBloqueadoPerfil(Integer codigoCliente, Integer relacaoTipo)
		...
		try {
			...
			consulta = "select distinct(imov.id) " + "from ClienteImovel clim " + "inner join clim.cliente clie "
					+ "inner join clim.imovel imov " + "inner join imov.imovelPerfil iper "
					+ "WHERE clim.dataFimRelacao is null and " + "iper.indicadorBloquearRetificacao = 1 and ";
			...
			resultado = session.createQuery(consulta).setInteger("codigoCliente", codigoCliente).list();
	...
	public Collection pesquisarImovelClientesEspeciaisRelatorio(
		...
		try {
			...
			// a ordena��o est� sendo feita atrav�s de comparator
			// pois alguns campos s� s�o obtidos ap�s a consulta
			...
			retorno = session.createSQLQuery(consulta).addScalar("idGerencia",
					...
					Hibernate.INTEGER).addScalar("nomeGerencia",
							...
							Hibernate.STRING)
					.addScalar("idLocalidade",

							Hibernate.INTEGER)
					.addScalar("nomeLocalidade",
					...
					.addScalar("idImovel",
							...
							Hibernate.INTEGER)
					.addScalar("idClienteUsuario",

							Hibernate.INTEGER)
					.addScalar("nomeClienteUsuario",

							Hibernate.STRING)
					.addScalar("qtdeEconomias",

							Hibernate.SHORT)
					.addScalar("sitLigAgua",

							Hibernate.STRING)
					.addScalar("sitLigEsg",

							Hibernate.STRING)
					.addScalar("hidrCapacidade",

							Hibernate.STRING)
					.addScalar("dtInstalacao",

							Hibernate.DATE)
					.addScalar("idClienteResponsavel",
					...
					.addScalar("consumoAgua", Hibernate.INTEGER).addScalar(
							...
							"consumoEsgoto", Hibernate.INTEGER)
					.addScalar(
					...
					.addScalar("consumoTarifa", Hibernate.STRING).addScalar(
							...
							"valorAgua", Hibernate.BIG_DECIMAL)
					.addScalar(

							"valorEsgoto", Hibernate.BIG_DECIMAL)
					.addScalar(

							"valorDebitos", Hibernate.BIG_DECIMAL)
					.addScalar(

							"valorCreditos", Hibernate.BIG_DECIMAL)
					.addScalar(

							"nomeSetor", Hibernate.STRING)
					.addScalar(

							"codRota", Hibernate.INTEGER)
					.addScalar(

							"iccobrancamulta", Hibernate.SHORT)
					.setInteger(
	...
	public Integer pesquisarImovelClientesEspeciaisRelatorioCount(String idUnidadeNegocio, String idGerenciaRegional,
		...
		try {
			...
			colecaoRetorno = session.createSQLQuery(consulta).addScalar("contador", Hibernate.INTEGER)
	...
	public String criarCondicionaisImovelClientesEspeciais(
		...
		if (Util.verificarNaoVazio(codigoSetorInicial)) {
			...
			sql = sql + " setor.stcm_cdsetorcomercial between " + codigoSetorInicial + " and " + codigoSetorFinal
					+ " and ";
		}
		...
		// Intervalo de Consumo por Respons�vel
		...
		if (Util.verificarNaoVazio(intervaloConsumoResponsavelInicial)) {
			...
			if (dataInstalacaoHidrometroInicial != null) {
				...
				sql = sql
						...
						+ dataInicial + "','YYYY-MM-DD HH24:MI:SS') and to_date('" + dataFinal
	...
	public FaturamentoGrupo pesquisarGrupoImovel(Integer idImovel) throws ErroRepositorioException {
		...
		try {
			...
			consulta = "select faturamentoGrupo from Imovel imovel"
					...
					+ " left join rotaAlternativa.faturamentoGrupo faturamentoGrupo" + " where imovel.id =:idImovel";

			retorno = (FaturamentoGrupo) session.createQuery(consulta).setInteger("idImovel", idImovel).setMaxResults(1)
					.uniqueResult();

			// Caso o imovel nao tenha rota alternativa retorna o grupo atraves da quadra
			if (retorno == null) {

				consulta = "select faturamentoGrupo from Imovel imovel" + " inner join imovel.quadra quadra"
						+ " inner join quadra.rota rota" + " inner join rota.faturamentoGrupo faturamentoGrupo"
						+ " where imovel.id =:idImovel";

				retorno = (FaturamentoGrupo) session.createQuery(consulta).setInteger("idImovel", idImovel)
						.setMaxResults(1).uniqueResult();
			}
		} catch (HibernateException e) {
			throw new ErroRepositorioException(e, "Erro no Hibernate");
	...
	public Collection gerarRelatorioImovelEnderecoOutrosCriterios(String idImovelCondominio, String idImovelPrincipal,
		...
		try {
			...
			consulta = consulta + montarCondicaoSQLWhereFiltrarImovelOutrosCriterio(idImovelCondominio,
					...
					quantidadeEconomiasFinal, diaVencimento, idCliente, idClienteTipo, idClienteRelacaoTipo,
					numeroPontosInicial, numeroPontosFinal, numeroMoradoresInicial, numeroMoradoresFinal,
					idAreaConstruidaFaixa, idUnidadeNegocio, ConstantesSistema.GERAR_RELATORIO_IMOVEL, rotaInicial,
					rotaFinal, seqRotaInicial, seqRotaFinal, null, null, null);

			SQLQuery query = null;
			if (ordenacaoRelatorio != null && ordenacaoRelatorio.equalsIgnoreCase("endereco")) {

				query = session.createSQLQuery(
						(consulta.substring(0, (consulta.length() - 5)).concat(" order by 22, 21 , 27 , 26 ")));

			} else if (ordenacaoRelatorio != null && !ordenacaoRelatorio.equals("")) {

				query = session.createSQLQuery(
						(consulta.substring(0, (consulta.length() - 5)).concat(" order by 2, 6, 83, 84")));

			} else {
				// loca, setor, rota , seq da rota, numero do lote e sub lote
				query = session.createSQLQuery(
						(consulta.substring(0, (consulta.length() - 5)).concat(" order by 2, 6, 83, 84 , 9, 10 ")));
			}
			...
			informarDadosQueryFiltrarImovelOutrosCriterio(query, idImovelCondominio, idImovelPrincipal,
					...
					idSubCategoria, idCategoria, quantidadeEconomiasInicial, quantidadeEconomiasFinal, diaVencimento,
					idCliente, idClienteTipo, idClienteRelacaoTipo, numeroPontosInicial, numeroPontosFinal,
					numeroMoradoresInicial, numeroMoradoresFinal, idAreaConstruidaFaixa, idUnidadeNegocio, rotaInicial,
					rotaFinal, seqRotaInicial, seqRotaFinal, null, null);

			retorno = query
					// .addScalar("imov_id",Hibernate.INTEGER)
					.addScalar("greg_id", Hibernate.INTEGER).addScalar("greg_nmabreviado", Hibernate.STRING)
					.addScalar("loca_id", Hibernate.INTEGER).addScalar("loca_nmlocalidade", Hibernate.STRING)
					.addScalar("imov_id", Hibernate.INTEGER).addScalar("imov_qteconomia", Hibernate.SHORT)
					.addScalar("stcm_cdsetorcomercial", Hibernate.INTEGER)
					.addScalar("stcm_nmsetorcomercial", Hibernate.STRING).addScalar("qdra_nnquadra", Hibernate.INTEGER)
					...
					.addScalar("lesg_nnconsumominimoesgoto", Hibernate.INTEGER)
					.addScalar("imov_icjardim", Hibernate.SHORT).addScalar("rota_cdrota", Hibernate.SHORT)
					.addScalar("imov_nnsequencialrota", Hibernate.INTEGER).list();
...
//					+ "rota.rota_id as idRota "
					 ...
					 * @date 06/08/2013
					 * 
					 *       Corre��o da nomenclatura do hidrometro_instalacao_historic (anterior
					 *       hidrometro_instalacao_historico_esgoto)
					 */
					+ "left join micromedicao.hidrometro_protecao as hidrometro_protecao_esgoto on hidrometro_instalacao_historic.hipr_id = hidrometro_protecao_esgoto.hipr_id ";
			...
			if (ordenacao != null && !ordenacao.trim().equals("")
					&& !ordenacao.trim().equals("" + ConstantesSistema.NUMERO_NAO_INFORMADO)) {
				if (ordenacao.trim().equals("rota")) {
					query = session.createSQLQuery(
							consulta.concat(" order by 1, 3, 7,codigoRota, imovel.imov_nnsequencialrota"));
//					query = session.createSQLQuery(consulta.concat(" order by idRota, sequencialRota"));
			...
			informarDadosQueryFiltrarImovelOutrosCriterio(query,
					...
					idUnidadeNegocio, cdRotaInicial, cdRotaFinal, sequencialRotaInicial, sequencialRotaFinal,
			...
			retorno = query
					...
					.addScalar("pisc_id", Hibernate.INTEGER).addScalar(
							...
							"poco_dspocotipo", Hibernate.STRING)
					.addScalar(
					...
					.addScalar("fttp_id", Hibernate.INTEGER).addScalar(
							...
							"logr_id", Hibernate.INTEGER)
					.addScalar(

							"cfon_cdddd", Hibernate.STRING)
					.addScalar(
	...
	public Collection<EmitirDocumentoCobrancaBoletimCadastroHelper> pesquisarBoletimCadastro(String idImovelCondominio,
			...
			String idClienteTipo, String idClienteRelacaoTipo, String numeroPontosInicial, String numeroPontosFinal,
			String numeroMoradoresInicial, String numeroMoradoresFinal, String idAreaConstruidaFaixa,
			String idUnidadeNegocio, int quantidadeCobrancaDocumentoInicio, String indicadorCpfCnpj, String cpfCnpj)
			throws ErroRepositorioException {
		...
		try {
			...
			consulta = consulta + "select distinct imovel.imov_id, localidade.loca_id, "// 0,1
			...
			consulta = consulta + montarCondicaoSQLWhereFiltrarImovelOutrosCriterio(idImovelCondominio,
					...
					quantidadeEconomiasFinal, diaVencimento, idCliente, idClienteTipo, idClienteRelacaoTipo,
					numeroPontosInicial, numeroPontosFinal, numeroMoradoresInicial, numeroMoradoresFinal,
					idAreaConstruidaFaixa, idUnidadeNegocio, ConstantesSistema.GERAR_RELATORIO_IMOVEL, null, null, null,
					null, null, indicadorCpfCnpj, cpfCnpj);

			consulta = consulta.substring(0, (consulta.length() - 5));

			consulta = consulta + " ORDER BY localidade.loca_id, " + "setor_comercial.stcm_cdsetorcomercial, "
					+ "quadra.qdra_nnquadra, imovel.imov_nnlote, " + "imovel.imov_nnsublote";
			...
			informarDadosQueryFiltrarImovelOutrosCriterio(query, idImovelCondominio, idImovelPrincipal,
					...
					idSubCategoria, idCategoria, quantidadeEconomiasInicial, quantidadeEconomiasFinal, diaVencimento,
					idCliente, idClienteTipo, idClienteRelacaoTipo, numeroPontosInicial, numeroPontosFinal,
					numeroMoradoresInicial, numeroMoradoresFinal, idAreaConstruidaFaixa, idUnidadeNegocio, null, null,
					null, null, null, cpfCnpj);
			Collection colecaoImovel = query.addScalar("imov_id", Hibernate.INTEGER)
					...
					.addScalar("imov_nnsublote", Hibernate.SHORT).addScalar("cbgr_id", Hibernate.INTEGER)
	...
	public CobrancaSituacao pesquisarImovelCobrancaSituacao(Integer idImovel) throws ErroRepositorioException {
		...
		try {
			...
			String consulta = "select iCS.cobrancaSituacao " + " from ImovelCobrancaSituacao iCS "
					+ " inner join iCS.imovel imovel " + " where imovel.id = :idImovel"
					+ " and iCS.dataRetiradaCobranca is null ";

			retorno = (CobrancaSituacao) session.createQuery(consulta).setInteger("idImovel", idImovel).setMaxResults(1)
					.uniqueResult();
	...
	public Short pesquisarObterQuantidadeEconomias(Imovel imovel, Categoria categoria) throws ErroRepositorioException {
		...
		try {
			...
			retorno = (Short) session.createQuery(consulta).setInteger("imovelId", imovel.getId().intValue())
	...
	public Imovel pesquisarDadosImovel(Integer idImovel) throws ErroRepositorioException {
		...
		try {
			...
			imovel = (Imovel) session.createQuery(consulta).setInteger("idImovel", idImovel).setMaxResults(1)
					.uniqueResult();
	...
	public Collection pesquisarInscricaoImoveleRota(String idsImovel) throws ErroRepositorioException {
		...
		try {
			...
			consulta = "SELECT imov.id, " + " loc.id, " + " setorCom.codigo, " + " quad.numeroQuadra, " + " imov.lote, "
					+ " imov.subLote, " + " imov.numeroSequencialRota, " + " rot.codigo, "
					+ " imov.quantidadeEconomias " + " FROM Imovel imov " + " LEFT JOIN imov.localidade loc "
					+ " LEFT JOIN imov.setorComercial setorCom " + " LEFT JOIN imov.quadra quad "
					+ " LEFT JOIN quad.rota rot " + " WHERE " + " imov.id in (" + idsImovel + ") " + " ORDER BY "
					+ " loc.id, " + " setorCom.codigo, " + " rot.codigo, " + " imov.numeroSequencialRota, "
					+ " quad.numeroQuadra, " + " imov.lote, " + " imov.subLote ";
	...
	public Collection pesquisarInscricaoImoveleRota(Collection colecaoIdsImovel) throws ErroRepositorioException {
		...
		try {
			...
			consulta = "SELECT imov.id, " + " loc.id, " + " setorCom.codigo, " + " quad.numeroQuadra, " + " imov.lote, "
					...
					+ " imov.quantidadeEconomias " + " FROM Imovel imov " + " LEFT JOIN imov.localidade loc "
					+ " LEFT JOIN imov.setorComercial setorCom " + " LEFT JOIN imov.quadra quad "
					+ " LEFT JOIN quad.rota rot " + " WHERE " + " imov.id in (:idsImovel) " + " ORDER BY " + " loc.id, "
					+ " setorCom.codigo, " + " rot.codigo, " + " imov.numeroSequencialRota, " + " quad.numeroQuadra, "
					+ " imov.lote, " + " imov.subLote ";

			retorno = session.createQuery(consulta).setParameterList("idsImovel", colecaoIdsImovel).list();
	...
	public Collection pesquisarInscricaoImoveleRota(
			FiltrarAnaliseExcecoesLeiturasHelper filtrarAnaliseExcecoesLeiturasHelper, Integer anoMes)
			throws ErroRepositorioException {
		...
		try {
			...
			sql = "SELECT imovel.imov_id as idImovel, " + " imovel.loca_id as idLocalidade, "
					+ " setorComercial.stcm_cdsetorcomercial as codigoSetorComercial, "
					+ " quadra.qdra_nnquadra as numeroQuadra, " + " imovel.imov_nnlote as lote, "
					+ " imovel.imov_nnsublote as subLote, " + " imovel.imov_nnsequencialrota as seqRota, "
					+ " rota.rota_cdrota as codigoRota, " + " imovel.imov_qteconomia as qtdEconomias, "
					+ " clieUsuario.clie_nmcliente as nomeUsuario " + " FROM cadastro.imovel imovel "
					+ " INNER JOIN cadastro.cliente_imovel clieImovUsuario "
					+ " on clieImovUsuario.imov_id = imovel.imov_id and clieImovUsuario.clim_dtrelacaofim is null and clieImovUsuario.crtp_id = "
					+ ClienteRelacaoTipo.USUARIO.toString() + " INNER JOIN cadastro.cliente clieUsuario "
					+ " on clieUsuario.clie_id = clieImovUsuario.clie_id "
					+ " INNER JOIN cadastro.setor_comercial setorComercial "
					+ " on imovel.stcm_id = setorComercial.stcm_id " + " INNER JOIN cadastro.quadra quadra "
					+ " on quadra.qdra_id = imovel.qdra_id " + " INNER JOIN micromedicao.rota rota "
			...
			if (filtrarAnaliseExcecoesLeiturasHelper.getIdMedicaoTipo() != null && !filtrarAnaliseExcecoesLeiturasHelper
				...
				if (filtrarAnaliseExcecoesLeiturasHelper.getIdMedicaoTipo().trim()
					...
					sql = sql + " INNER JOIN micromedicao.medicao_historico medHistAgua "
							+ " on imovel.imov_id = medHistAgua.lagu_id and medHistAgua.mdhi_amleitura = " + anoMes
							+ " and medHistAgua.medt_id = " + MedicaoTipo.LIGACAO_AGUA
					...
					sql = sql + " INNER JOIN micromedicao.medicao_historico medHistEsgoto "
							+ " on imovel.imov_id = medHistEsgoto.imov_id and medHistEsgoto.mdhi_amleitura = " + anoMes
							+ " and medHistEsgoto.medt_id = " + MedicaoTipo.POCO;
				}
			} else {
				sql = sql + " LEFT OUTER JOIN micromedicao.medicao_historico medHistAgua "
						+ " on imovel.imov_id = medHistAgua.lagu_id and medHistAgua.mdhi_amleitura = " + anoMes
						+ " and medHistAgua.medt_id = " + MedicaoTipo.LIGACAO_AGUA
			...
			if (filtrarAnaliseExcecoesLeiturasHelper.getIdLigacaoTipo() != null && !filtrarAnaliseExcecoesLeiturasHelper
					.getIdLigacaoTipo().trim().equalsIgnoreCase("" + ConstantesSistema.NUMERO_NAO_INFORMADO)) {
				if (filtrarAnaliseExcecoesLeiturasHelper.getIdLigacaoTipo().trim()
						.equals("" + LigacaoTipo.LIGACAO_AGUA)) {
					sql = sql + " INNER JOIN micromedicao.consumo_historico consHistAgua "
					...
					sql = sql + " INNER JOIN micromedicao.consumo_historico consHistEsgoto "
				...
				sql = sql + " LEFT OUTER JOIN micromedicao.consumo_historico consHistAgua "
						+ " on consHistAgua.imov_id = imovel.imov_id and consHistAgua.cshi_amfaturamento = :anoMes and consHistAgua.lgti_id = "
						+ LigacaoTipo.LIGACAO_AGUA + " LEFT OUTER JOIN micromedicao.consumo_historico consHistEsgoto "
						+ " on consHistEsgoto.imov_id = imovel.imov_id and consHistEsgoto.cshi_amfaturamento = :anoMes and consHistEsgoto.lgti_id = "
						+ LigacaoTipo.LIGACAO_ESGOTO
						+ " LEFT OUTER JOIN micromedicao.consumo_anormalidade consAnormAgua "
						+ " on consAnormAgua.csan_id = consHistAgua.csan_id "
			...
			sql = sql + " INNER JOIN cadastro.logradouro_bairro logrBairro "
					+ " on logrBairro.lgbr_id = imovel.lgbr_id " + " INNER JOIN cadastro.bairro bairro "
					+ " on bairro.bair_id = logrBairro.bair_id " + " INNER JOIN cadastro.municipio municipio "
					...
					+ " ORDER BY " + " imovel.loca_id, " + " setorComercial.stcm_cdsetorcomercial, "
					+ " rota.rota_cdrota, " + " imovel.imov_nnsequencialrota, " + " quadra.qdra_nnquadra, "
					+ " imovel.imov_nnlote, " + " imovel.imov_nnsublote ";
			...
			retorno = session.createSQLQuery(sql).addScalar("idImovel", Hibernate.INTEGER)
					...
					.addScalar("subLote", Hibernate.SHORT).addScalar("seqRota", Hibernate.INTEGER)
					.addScalar("codigoRota", Hibernate.SHORT).addScalar("qtdEconomias", Hibernate.SHORT)
					.addScalar("nomeUsuario", Hibernate.STRING).setInteger("anoMes", anoMes).list();
...
//			+ " imovel.imov_icexclusao = " + ConstantesSistema.INDICADOR_IMOVEL_ATIVO;
		...
		if (filtrarAnaliseExcecoesLeiturasHelper.getIdImovel() != null
				&& !filtrarAnaliseExcecoesLeiturasHelper.getIdImovel().trim().equals("")) {
		...
		if (filtrarAnaliseExcecoesLeiturasHelper.getIdImovelCondominio() != null
				&& !filtrarAnaliseExcecoesLeiturasHelper.getIdImovelCondominio().trim().equals("")) {
			retorno = retorno + " and imovel.imov_idimovelcondominio = "
					+ filtrarAnaliseExcecoesLeiturasHelper.getIdImovelCondominio() + " or imovel.imov_id = "
					+ filtrarAnaliseExcecoesLeiturasHelper.getIdImovelCondominio();
		}

		if (filtrarAnaliseExcecoesLeiturasHelper.getIdFaturamentoGrupo() != null
				&& !filtrarAnaliseExcecoesLeiturasHelper.getIdFaturamentoGrupo().trim().equals("")) {
		...
		if (filtrarAnaliseExcecoesLeiturasHelper.getIdEmpresa() != null
				&& !filtrarAnaliseExcecoesLeiturasHelper.getIdEmpresa().trim().equals("")) {
		...
		if (filtrarAnaliseExcecoesLeiturasHelper.getIdLocalidade() != null
				&& !filtrarAnaliseExcecoesLeiturasHelper.getIdLocalidade().trim().equals("")) {
		...
		if (filtrarAnaliseExcecoesLeiturasHelper.getCodigoSetorComercial() != null
				&& !filtrarAnaliseExcecoesLeiturasHelper.getCodigoSetorComercial().trim().equals("")) {
			retorno = retorno + " and setorComercial.stcm_cdsetorcomercial = "
					+ filtrarAnaliseExcecoesLeiturasHelper.getCodigoSetorComercial();
		}

		if (filtrarAnaliseExcecoesLeiturasHelper.getNumeroQuadraInicial() != null
				&& !filtrarAnaliseExcecoesLeiturasHelper.getNumeroQuadraInicial().trim().equals("")) {
			retorno = retorno + " and quadra.qdra_nnquadra between "
					+ filtrarAnaliseExcecoesLeiturasHelper.getNumeroQuadraInicial() + " and "
					+ filtrarAnaliseExcecoesLeiturasHelper.getNumeroQuadraFinal();
		}

		if (filtrarAnaliseExcecoesLeiturasHelper.getCodigoRota() != null
				&& !filtrarAnaliseExcecoesLeiturasHelper.getCodigoRota().trim().equals("")) {
		...
		if (filtrarAnaliseExcecoesLeiturasHelper.getIdUsuarioAlteracao() != null
				&& !filtrarAnaliseExcecoesLeiturasHelper.getIdUsuarioAlteracao().trim().equals("")) {

			String idUsuario = filtrarAnaliseExcecoesLeiturasHelper.getIdUsuarioAlteracao();

			if (filtrarAnaliseExcecoesLeiturasHelper.getIdMedicaoTipo() != null && !filtrarAnaliseExcecoesLeiturasHelper
					.getIdMedicaoTipo().equals("" + ConstantesSistema.NUMERO_NAO_INFORMADO)) {
				if (filtrarAnaliseExcecoesLeiturasHelper.getIdMedicaoTipo().trim()
						.equals("" + MedicaoTipo.LIGACAO_AGUA)) {
				...
				retorno = retorno + " and (medHistAgua.usur_idalteracao = " + idUsuario
						+ " or medHistEsgoto.usur_idalteracao = " + idUsuario + ") ";
			}

		}

		if (filtrarAnaliseExcecoesLeiturasHelper.getIndicadorImovelCondominio() != null
				&& !filtrarAnaliseExcecoesLeiturasHelper.getIndicadorImovelCondominio().trim().equals("")) {
			retorno = retorno + " and imovel.imov_icimovelcondominio = "
					+ filtrarAnaliseExcecoesLeiturasHelper.getIndicadorImovelCondominio();
		}

		if (filtrarAnaliseExcecoesLeiturasHelper.getIndicadorDebitoAutomatico() != null
				&& !filtrarAnaliseExcecoesLeiturasHelper.getIndicadorDebitoAutomatico().trim().equals("")) {
			retorno = retorno + " and imovel.imov_icdebitoconta = "
					+ filtrarAnaliseExcecoesLeiturasHelper.getIndicadorDebitoAutomatico();
		}
		...
		if (filtrarAnaliseExcecoesLeiturasHelper.getIndicadorAnalisado() != null) {
			...
			for (int i = 0; i < indicadorAnalisado.length; i++) {
				if (indicadorAnalisado[i] != null
						&& !indicadorAnalisado[i].trim().equals("" + ConstantesSistema.NUMERO_NAO_INFORMADO)) {
			...
			if (filtrarAnaliseExcecoesLeiturasHelper.getIdMedicaoTipo() != null && !filtrarAnaliseExcecoesLeiturasHelper
				...
				retorno = retorno + " and ( medHistAgua.mdhi_icanalisado in ( " + valores
						+ ") or medHistEsgoto.mdhi_icanalisado in ( " + valores + " ) ) ";
			}
		...
		if (filtrarAnaliseExcecoesLeiturasHelper.getIdsImovelPerfil() != null) {
			...
			for (int i = 0; i < idsImovelPerfil.length; i++) {
				if (idsImovelPerfil[i] != null
						&& !idsImovelPerfil[i].trim().equals("" + ConstantesSistema.NUMERO_NAO_INFORMADO)) {
		...
		if (filtrarAnaliseExcecoesLeiturasHelper.getIdCategoria() != null
				&& !filtrarAnaliseExcecoesLeiturasHelper.getIdCategoria().trim().equals("")) {
		...
		if (filtrarAnaliseExcecoesLeiturasHelper.getQuantidadeEconomias() != null
				&& !filtrarAnaliseExcecoesLeiturasHelper.getQuantidadeEconomias().trim().equals("")) {
			retorno = retorno + " and imovel.imov_qteconomia = "
					+ filtrarAnaliseExcecoesLeiturasHelper.getQuantidadeEconomias();
		}
		...
		if (filtrarAnaliseExcecoesLeiturasHelper.getIdTipoAnormalidade() != null) {
			...
			if (idTipoAnormalidade.equals(FiltrarAnaliseExcecoesLeiturasHelper.ANORMALIDADE_LEITURA_INFORMADA)) {
				if (filtrarAnaliseExcecoesLeiturasHelper.getIdMedicaoTipo() != null
						&& !filtrarAnaliseExcecoesLeiturasHelper.getIdMedicaoTipo()
								.equals("" + ConstantesSistema.NUMERO_NAO_INFORMADO)) {
					if (filtrarAnaliseExcecoesLeiturasHelper.getIdMedicaoTipo().trim()
							.equals("" + MedicaoTipo.LIGACAO_AGUA)) {
				...
				if (filtrarAnaliseExcecoesLeiturasHelper.getIdLigacaoTipo() != null
						&& !filtrarAnaliseExcecoesLeiturasHelper.getIdLigacaoTipo()
								.equals("" + ConstantesSistema.NUMERO_NAO_INFORMADO)) {
					if (filtrarAnaliseExcecoesLeiturasHelper.getIdLigacaoTipo().trim()
							.equals("" + LigacaoTipo.LIGACAO_AGUA)) {
		...
		if (filtrarAnaliseExcecoesLeiturasHelper.getIdsAnormalidadeLeituraInformada() != null) {
			...
			if (valor != null && !valor.equals("")) {
				...
				if (filtrarAnaliseExcecoesLeiturasHelper.getIdMedicaoTipo() != null
					...
					retorno = retorno + " and (medHistAgua.ltan_idleitanorminformada in (" + valor + ")"
							+ " or medHistEsgoto.ltan_idleitanorminformada in (" + valor + ") ) ";
				}
			}
		}
		...
		if (filtrarAnaliseExcecoesLeiturasHelper.getIdsAnormalidadeLeituraFaturada() != null) {
			...
			if (valor != null && !valor.equals("")) {
				...
				if (filtrarAnaliseExcecoesLeiturasHelper.getIdMedicaoTipo() != null
					...
					retorno = retorno + " and (medHistAgua.ltan_idleitanormfatmt in (" + valor + ")"
							+ " or medHistEsgoto.ltan_idleitanormfatmt in (" + valor + ") ) ";
				}
			}
		...
		if (filtrarAnaliseExcecoesLeiturasHelper.getIdsAnormalidadeConsumo() != null) {
			...
			if (valor != null && !valor.equals("")) {
				...
				if (filtrarAnaliseExcecoesLeiturasHelper.getIdLigacaoTipo() != null
					...
					retorno = retorno + " and (consHistAgua.csan_id in (" + valor + ")"
							+ " or consHistEsgoto.csan_id in (" + valor + ") ) ";
				}
			}
		...
		if (filtrarAnaliseExcecoesLeiturasHelper.getConsumoFaturadoInicial() != null
				&& !filtrarAnaliseExcecoesLeiturasHelper.getConsumoFaturadoInicial().trim().equals("")) {
			...
			if (filtrarAnaliseExcecoesLeiturasHelper.getIdLigacaoTipo() != null && !filtrarAnaliseExcecoesLeiturasHelper
					.getIdLigacaoTipo().equals("" + ConstantesSistema.NUMERO_NAO_INFORMADO)) {
				if (filtrarAnaliseExcecoesLeiturasHelper.getIdLigacaoTipo().trim()
						.equals("" + LigacaoTipo.LIGACAO_AGUA)) {
		...
		if (filtrarAnaliseExcecoesLeiturasHelper.getConsumoMedidoInicial() != null
				&& !filtrarAnaliseExcecoesLeiturasHelper.getConsumoMedidoInicial().trim().equals("")) {
		...
		if (filtrarAnaliseExcecoesLeiturasHelper.getConsumoMedioInicial() != null
				&& !filtrarAnaliseExcecoesLeiturasHelper.getConsumoMedioInicial().trim().equals("")) {
		...
		if (filtrarAnaliseExcecoesLeiturasHelper.getIdLeituraSituacaoAtual() != null
				&& !filtrarAnaliseExcecoesLeiturasHelper.getIdLeituraSituacaoAtual().trim().equals("")) {
			...
			if (filtrarAnaliseExcecoesLeiturasHelper.getIdMedicaoTipo() != null && !filtrarAnaliseExcecoesLeiturasHelper
				...
				retorno = retorno + " and (medHistAgua.ltst_idleiturasituacaoatual = " + idLeituraSituacao
						+ " or medHistEsgoto.ltst_idleiturasituacaoatual >= " + idLeituraSituacao + ") ";
			}
	...
	public Collection pesquisarImoveisClientesRelacao(Collection idsCliente,
		...
		try {
			...
			consulta = "select distinct im.id, rt.id, "
					...
					+ "where cl.id in (:idsClientes) and ci.dataFimRelacao is null" + " order by im.id";
	...
	public void atualizarSituacaoCobrancaImovel(Integer idSituacaoCobranca, Integer idImovel)
		...
		try {
			...
			session.createQuery(consulta).setInteger("idSituacaoCobranca", idSituacaoCobranca)
	 ...
	 * @throws ErroRepositorioException
	 */
	public Object pesquisarQuantidadeImovelInscricao(String idImovel, String idLocalidade, String codigoSetorComercial,
...
//				}
			...
			Query query = session.createSQLQuery(consulta.substring(0, (consulta.length() - 5))).addScalar("qtd",
					Hibernate.INTEGER);
			// seta os valores na condi��o where
			// imovel principal
			if (idImovel != null && !idImovel.equals("") && !idImovel.trim()
					.equalsIgnoreCase(new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO).toString())) {
			...
			// localidade
			if ((idLocalidade != null && !idLocalidade.equals("") && !idLocalidade.trim()
					.equalsIgnoreCase(new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO).toString()))) {
			...
			// setor
			if ((codigoSetorComercial != null && !codigoSetorComercial.equals("") && !codigoSetorComercial.trim()
					.equalsIgnoreCase(new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO).toString()))) {
			...
			// quadra
			if ((numeroQuadra != null && !numeroQuadra.equals("") && !numeroQuadra.trim()
					.equalsIgnoreCase(new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO).toString()))) {
			...
			if ((lote != null && !lote.equals("")
					&& !lote.trim().equalsIgnoreCase(new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO).toString()))) {
			...
			// cliente
			if (codigoCliente != null && !codigoCliente.equals("") && !codigoCliente.trim()
					.equalsIgnoreCase(new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO).toString())) {
			...
			// municipio
			if (idMunicipio != null && !idMunicipio.equals("") && !idMunicipio.trim()
					.equalsIgnoreCase(new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO).toString())) {
			...
			// bairro
			if (idBairro != null && !idBairro.equals("") && !idBairro.trim()
					.equalsIgnoreCase(new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO).toString())) {
			...
			// logradouro
			if (idLogradouro != null && !idLogradouro.equals("") && !idLogradouro.trim()
					.equalsIgnoreCase(new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO).toString())) {
	...
	public Object[] pesquisarAreaConstruida(Integer idImovel) throws ErroRepositorioException {
		...
		try {
			consulta = "SELECT imov.areaConstruida, acon.menorFaixa "// 0, 1
					+ "FROM Imovel imov " + "LEFT JOIN imov.areaConstruidaFaixa acon " + "WHERE imov.id = :idImovel ";

			retorno = (Object[]) session.createQuery(consulta).setInteger("idImovel", idImovel).setMaxResults(1)
					.uniqueResult();
	...
	public Integer obterIdEsferaPoder(Integer idImovel) throws ErroRepositorioException {
		...
		try {
			...
			consulta = "SELECT ct.epod_id as idEsferaPoder FROM cadastro.cliente_imovel ci, "
					...
					+ " where ci.clie_id = c.clie_id and c.cltp_id = ct.cltp_id " + " and ci.crtp_id = "
					+ ClienteRelacaoTipo.USUARIO + " and ci.imov_id = " + idImovel;

			retorno = session.createSQLQuery(consulta).addScalar("idEsferaPoder", Hibernate.INTEGER).setMaxResults(1)
					.list();
	...
	/**
	 * Consulta o ID da categoria principal, considerando que a categoria principal
	 * � a quem tem maior soma de qtd de economias em suas subcategorias e tenha o
	...
	public Integer obterIdCategoriaPrincipal(Integer idImovel, boolean ordemDecrescente)
		...
		try {
			...
			consulta = "SELECT sc.catg_id as idCategoria, count(isc.imsb_qteconomia) as quantidade FROM "
					+ "cadastro.subcategoria sc, " + "cadastro.categoria c , " + "cadastro.imovel_subcategoria isc "
					+ "where sc.catg_id = c.catg_id and isc.scat_id = sc.scat_id "
					+ "and isc.imov_id = :idImovel group by sc.catg_id " + "order by quantidade DESC, idCategoria "
	...
	public Collection pesquisarImovelPerfilDiferenteCorporativo() throws ErroRepositorioException {
		...
		try {
			...
			consulta = " SELECT iper " + " FROM ImovelPerfil iper " + " WHERE iper.indicadorUso = :indicadorUso "
			...
			retorno = (Collection) session.createQuery(consulta)
					...
					.setInteger("corporativoTelemed", ImovelPerfil.CORPORATIVO_TELEMED).list();
	...
	public List pesquisarLocalidadeComImoveisComSituacaoLigadoEmAnalise() throws ErroRepositorioException {
		...
		try {
			String hql = " select distinct(l.id) " + " from gcom.cadastro.imovel.Imovel imov "
					+ " left join imov.localidade l " + " where " + " imov.ligacaoAguaSituacao.id = 4 ";
	...
	/**
	 * [UC0823] Atualiza Liga��o de �gua de Ligado em An�lise para Ligado Seleciona
	 * a lista de im�veis que esteja com a situa��o de �gua ligado em an�lise.
	 * 
	 * @author Yara Taciane
	 * @date 23/05/2008
	 */
	public Collection pesquisarImoveisComSituacaoLigadoEmAnalise(Integer idLocalidade) throws ErroRepositorioException {
		...
		try {
			String sql = " select imov.imov_id as idImovel" + " from cadastro.imovel imov " + " where  imov.last_id in "
					+ "( " + LigacaoAguaSituacao.LIGADO_EM_ANALISE + "," + LigacaoAguaSituacao.CORTADO + ")" + "and"
					+ " imov.loca_id = " + idLocalidade;

			Collection coll = (Collection) session.createSQLQuery(sql).addScalar("idImovel", Hibernate.INTEGER).list();
	...
	/**
	 * [UC0823] Atualizar Situa��o �gua Para Ligado Auhtor: Yara Taciane Data:
	 * 07/07/2008
	 * 
	 * @param id                 Matricula do Imovel
	 * @param situacaoAguaLigado Situa��o Agua
	 * 
	 * @exception ErroRepositorioException Repositorio Exception
	 */
	public void atualizarSituacaoAguaPorImovel(String id, String situacaoAguaLigado) throws ErroRepositorioException {
		...
		try {
			...
			session.createQuery(atualizarImovel).setInteger("situacaoAgua", new Integer(situacaoAguaLigado).intValue())
					.setTimestamp("ultimaAlteracao", new Date()).setInteger("id", new Integer(id).intValue())
					.executeUpdate();
	 ...
	 *
	 * Pesquisar as contas pertencentes ao imovel e anoMes informados que n�o
	 * estejam com a situa��o atual igual a "PR�-FATURADA"
	 *
	 * @author Raphael Rossiter
	 * @date 15/08/2008
	 *
	...
	public Object pesquisarImovelIdComContaNaoPreFaturada(Integer imovelId,
		...
		try {
			...
			consulta = "select ct.id from Conta ct "
					...
					+ "inner join ct.imovel im " + "inner join ct.debitoCreditoSituacaoAtual situacaoAtual "
	...
	public Integer obterQuadraPerfil(Integer idImovel) throws ErroRepositorioException {
		...
		try {
			consulta = " select qdpf_id as perfilQuadra" + " from cadastro.imovel i"
					+ " inner join cadastro.quadra q on(i.qdra_id = q.qdra_id)" + " where i.imov_id = " + idImovel;

			retorno = (Integer) session.createSQLQuery(consulta).addScalar("perfilQuadra", Hibernate.INTEGER)
					.uniqueResult();
	...
	public void atualizarImovelSituacaoAtualizacaoCadastral(Integer idImovel, Integer idSituacaoAtualizacaoCadastral)
		...
		try {
			...
			update = "update gcom.cadastro.imovel.Imovel set " + "siac_id = :idSituacaoAtualizacaoCadastral, "
					+ "imov_tmultimaalteracao = :datahoracorrente " + "where imov_id = :idImovel ";
	...
	public void atualizarImovelAtualizacaoCadastralSituacaoAtualizacaoCadastral(Integer idImovel,
		...
		try {
			...
			update = "update gcom.cadastro.imovel.ImovelAtualizacaoCadastral set "
					+ "siac_id = :idSituacaoAtualizacaoCadastral, " + "imac_tmultimaalteracao = :datahoracorrente ";
			...
			if (idEmpresa != null && !idEmpresa.equals(ConstantesSistema.NUMERO_NAO_INFORMADO)) {
				session.createQuery(update).setInteger("idImovel", idImovel)
						.setInteger("idSituacaoAtualizacaoCadastral", idSituacaoAtualizacaoCadastral)
						.setTimestamp("datahoracorrente", new Date()).setInteger("idEmpresa", idEmpresa)
						.executeUpdate();
			} else {
				session.createQuery(update).setInteger("idImovel", idImovel)
						.setInteger("idSituacaoAtualizacaoCadastral", idSituacaoAtualizacaoCadastral)
	...
	public Integer verificaExistenciaImovelAtualizacaoCadastral(Integer idImovel) throws ErroRepositorioException {
		...
		try {
			...
			consulta = "select imov_id as id " + "from cadastro.imovel_atlz_cadastral " + "where imov_id = :idImovel  ";

			retornoConsulta = (Integer) session.createSQLQuery(consulta).addScalar("id", Hibernate.INTEGER)
					.setInteger("idImovel", idImovel).setMaxResults(1).uniqueResult();
	...
	public Collection pesquisarImovelEconomia(Integer idImovel, Integer idSubcategoria)
		...
		try {
			...
			consulta = " select imec " + " from ImovelEconomia imec "
					...
					+ " left join fetch imec.imovelSubcategoria imsb " + " left join fetch imsb.comp_id.imovel imov "
					+ " left join fetch imov.imovelPerfil iper " + " left join fetch imsb.comp_id.subcategoria scat "
					+ " left join fetch scat.categoria " + " where imov.id = :idImovel "
	...
	public ImovelAtualizacaoCadastral pesquisarImovelAtualizacaoCadastralInscricao(Integer idImovel, Integer idEmpresa)
		...
		try {
			...
			consulta = " SELECT imov" + " FROM ImovelAtualizacaoCadastral imov" + " WHERE imov.idImovel = :idImovel"
	...
	public Collection obterImoveisAtualizacaoCadastral(Integer idLocalidade, String codigoSetor, Integer quadraInicial,
			Integer quadraFinal, Integer idEmpresa, Integer idRota) throws ErroRepositorioException {
		...
		try {
			consulta = " SELECT imov.id " + " FROM ImovelAtualizacaoCadastral imov" + " WHERE imov.idEmpresa ="
					+ idEmpresa + " AND imov.idSituacaoAtualizacaoCadastral=" + SituacaoAtualizacaoCadastral.DISPONIVEL
			...
			if (quadraInicial != null) {
				...
				consulta = consulta + " AND imov.numeroQuadra in" + "           (SELECT qdra.numeroQuadra"
						+ "            FROM Quadra qdra" + "            WHERE qdra.rota.id = " + idRota + ")";
			}
	...
	public Boolean pesquisarExistenciaImovelEconomia(Integer idImovel, Integer idSubcategoria)
		...
		try {
			...
			consulta = " select imec.id " + " from ImovelEconomia imec " + " left join imec.imovelSubcategoria imsb "
					+ " left join imsb.comp_id.imovel imov " + " left join imsb.comp_id.subcategoria scat "
					+ " where imov.id = :idImovel " + " and scat.id = :idSubcategoria ";

			Collection colecaoConsulta = session.createQuery(consulta).setInteger("idImovel", idImovel)
					.setInteger("idSubcategoria", idSubcategoria).list();
	...
	public Boolean verificarExistenciaTarifaConsumoCategoriaParaCategoriaImovel(Integer idImovel, Integer idCategoria)
		...
		try {
			...
			consulta = "select cstc.catg_id as idCategoria " + "from cadastro.imovel imov "
					...
					+ "where cstf_id = imov.cstf_id) and cstc.catg_id = :idCategoria " + "group by cstc.catg_id ";

			Collection colecaoConsulta = session.createSQLQuery(consulta).addScalar("idCategoria", Hibernate.INTEGER)
					.setInteger("idImovel", idImovel).setInteger("idCategoria", idCategoria).list();

			/*
			 * Collection colecaoConsulta = session.createSQLQuery(consulta).addScalar(
	...
	public void removerImovelSubcategoria(Integer idImovel) throws ErroRepositorioException {
		...
		try {
			remocao = "delete ImovelSubcategoria" + " where imov_id = :idImovel";

			session.createQuery(remocao).setInteger("idImovel", idImovel).executeUpdate();
	...
	public void removerImovelRamoAtividade(Integer idImovel) throws ErroRepositorioException {
		...
		try {
			...
			remocao = "delete ImovelRamoAtividade" + " where imov_id = :idImovel";
	...
	public Integer pesquisarMaiorNumeroLoteDaQuadra(Integer idQuadra) throws ErroRepositorioException {
		...
		try {
			...
			Integer resultado = (Integer) session.createSQLQuery(consulta).addScalar("max", Hibernate.INTEGER)
	...
	public Object[] consultarDadosClienteUsuarioImovel(String idImovel) throws ErroRepositorioException {
		...
		try {
			...
			retorno = (Object[]) session.createQuery(consulta).setInteger("idImovel", new Integer(idImovel))
					.setShort("idClienteUsuario", ClienteRelacaoTipo.USUARIO.shortValue()).uniqueResult();
		...
		try {
			...
			if (idsImovel != null && idsImovel.size() > 1000) {
				...
				for (Integer idImovel : idsImovel) {
					...
					if (idsImovelMenor.size() >= 1000) {
						...
						session.createQuery(consulta).setInteger("idSituacaoCobranca", idSituacaoCobranca)
								...
								.setTimestamp("ultimaAlteracao", new Date()).setParameterList("ids", idsImovelMenor)
								.executeUpdate();
				...
				if (idsImovel != null) {
					...
					session.createQuery(consulta).setInteger("idSituacaoCobranca", idSituacaoCobranca)
							.setInteger("idSituacaoCobrancaTipo", idSituacaoCobrancaTipo)
							.setTimestamp("ultimaAlteracao", new Date()).setParameterList("ids", idsImovel)
							.executeUpdate();
				}
			}
	...
	public Integer gerarRelatorioCadastroConsumidoresInscricaoCount(String idImovelCondominio, String idImovelPrincipal,
			...
			String numeroMoradoresInicial, String numeroMoradoresFinal, String idAreaConstruidaFaixa,
			String idUnidadeNegocio, String cdRotaInicial, String cdRotaFinal, String sequencialRotaInicial,
			String sequencialRotaFinal, String[] pocoTipoList) throws ErroRepositorioException {
		...
		try {
			...
			consulta = consulta
					...
					+ "select count(distinct " + "imovel.imov_id ) as qtde "
					+ "from cadastro.imovel_subcategoria imovelSubcategoria "
					+ "inner join cadastro.imovel on imovelSubcategoria.imov_id = cadastro.imovel.imov_id "
					+ "inner join cadastro.localidade on imovel.loca_id = cadastro.localidade.loca_id "
					+ "inner join cadastro.gerencia_regional on cadastro.localidade.greg_id = cadastro.gerencia_regional.greg_id "
			...
			consulta = consulta + montarCondicaoSQLWhereFiltrarImovelOutrosCriterio(idImovelCondominio,
					...
					numeroPontosInicial, numeroPontosFinal, numeroMoradoresInicial, numeroMoradoresFinal,
					idAreaConstruidaFaixa, idUnidadeNegocio, ConstantesSistema.GERAR_RELATORIO_IMOVEL, cdRotaInicial,
					cdRotaFinal, sequencialRotaInicial, sequencialRotaFinal, pocoTipoList, null, null);

			SQLQuery query = session.createSQLQuery(consulta.substring(0, (consulta.length() - 5)).concat(" "));
			...
			informarDadosQueryFiltrarImovelOutrosCriterio(query, idImovelCondominio, idImovelPrincipal,
					...
					idCliente, idClienteTipo, idClienteRelacaoTipo, numeroPontosInicial, numeroPontosFinal,
					numeroMoradoresInicial, numeroMoradoresFinal, idAreaConstruidaFaixa, idUnidadeNegocio,
					cdRotaInicial, cdRotaFinal, sequencialRotaInicial, sequencialRotaFinal, pocoTipoList, null);

			retorno = (Integer) query.addScalar("qtde", Hibernate.INTEGER).uniqueResult();
	...
	public Integer gerarRelatorioImovelEnderecoOutrosCriteriosCount(String idImovelCondominio, String idImovelPrincipal,
			...
			String seqRotaInicial, String seqRotaFinal) throws ErroRepositorioException {
		...
		try {
			...
			consulta = consulta + "select count(distinct " + "imovel.imov_id )as qtde "
					+ "from cadastro.imovel_subcategoria imovelSubcategoria "
					+ "inner join cadastro.imovel on imovelSubcategoria.imov_id = cadastro.imovel.imov_id "
					+ "inner join cadastro.localidade on imovel.loca_id = cadastro.localidade.loca_id "
					+ "inner join cadastro.gerencia_regional on cadastro.localidade.greg_id = cadastro.gerencia_regional.greg_id "
	...
	public Boolean verificarImovelPerfilBloqueado(Integer idImovel) throws ErroRepositorioException {
		...
		try {
			...
			consulta = " select imov.imov_id as id " + " from cadastro.imovel imov"
	...
	public Date pesquisarUltimaAlteracaoImovel(Integer idImovel) throws ErroRepositorioException {
		...
		try {
			...
			consulta = " select imov.imov_tmultimaalteracao as ultimaAlteracao " + " from cadastro.imovel imov"
					+ " where imov.imov_id = :idImovel ";

			retorno = (Date) session.createSQLQuery(consulta).addScalar("ultimaAlteracao", Hibernate.TIMESTAMP)
					.setInteger("idImovel", idImovel).uniqueResult();
	...
	public Integer pesquisarImovelComNumeroMedidorEnergiaImovel(String numeroMedidorEnergia)
		...
		try {
			...
			retorno = (Integer) session.createSQLQuery(consulta).addScalar("idImovel", Hibernate.INTEGER)
	...
	public Collection pesquisarCategoriaSubcategoriaImovel(Integer idImovel)
		...
		try {
			...
			consulta = "select categoria.id, "
					...
					+ "where imovelSubcategoria.comp_id.imovel.id = :idImovel " + "order by categoria.id";
	...
	public Integer obterConsumoFaturadoImovelNoMes(Integer idImovel, Integer mesAnoReferencia)
		...
		try {
			...
			consulta = " select ch.numeroConsumoFaturadoMes " + " from ConsumoHistorico ch "
					+ " inner join ch.ligacaoTipo lt" + " where ch.imovel = :idImovel"
					+ " and ch.referenciaFaturamento = :mesAnoReferencia and lt.id = :ligacaoAgua";

			retorno = (Integer) session.createQuery(consulta).setParameter("idImovel", idImovel)
					.setParameter("mesAnoReferencia", mesAnoReferencia)
					.setParameter("ligacaoAgua", LigacaoTipo.LIGACAO_AGUA).uniqueResult();
	...
	/**
	 * Este m�todo est� de acordo com o [UC0979] Gerar Relat�rio de Im�veis em
	 * Programas Especiais.
	 * 
	 ...
	 * @throws ErroRepositorioException
	 */
	public List filtrarImoveisProgramasEspeciais(FiltrarRelatorioImoveisProgramasEspeciaisHelper filtro)
			throws ErroRepositorioException {
		List retorno = null;
		...
		try {
			...
			if (filtro.getIdRegiaoDesenvolvimento() != null && !filtro.getIdRegiaoDesenvolvimento().equals("")) {
				consulta += "e.id = " + filtro.getIdRegiaoDesenvolvimento() + " and ";
			}
			...
			consulta += " group by " + "d.nomeAbreviado, d.nome, " + "e.nome, b.descricao " + "order by "
	...
	public ImovelPerfil recuperaPerfilImovel(Integer idImovel) throws ErroRepositorioException {
		...
		try {
			...
			consulta = " select imovelPerfil "
					...
					+ " where imovel.imovelPerfil.id = imovelPerfil.id " + " and imovel.id = :idImovel ";

			retorno = (ImovelPerfil) session.createQuery(consulta).setParameter("idImovel", idImovel).setMaxResults(1)
					.uniqueResult();
	 ...
	 * 
	 * M�todo criado para atualizar apenas os campos necess�rios para imovel.
	 * 
	 * @author Bruno Barros
	 * @date 31/03/2010
	 * @param imovel
	...
	public void atualizarImovelLeituraAnormalidadeProcessoMOBILE(Imovel imovel) throws ErroRepositorioException {
		...
		try {
			...
			String update = "update " + " cadastro.imovel " + " set ltan_id = ?, " + // 1
	...
	public void atualizarDataRetiradaImovelSituacaoCobranca(Integer idImovelSituacaoCobranca, Date dataRetirada)
		...
		try {
			...
			consulta = "update gcom.cadastro.imovel.ImovelCobrancaSituacao set "
					+ "iscb_dtretiradacobranca = :dataRetirada , " + " iscb_tmultimaalteracao = :ultimaAlteracao "
					+ "where iscb_id = :idImovelSituacaoCobranca ";

			session.createQuery(consulta).setDate("dataRetirada", dataRetirada)
					.setTimestamp("ultimaAlteracao", new Date())
					.setInteger("idImovelSituacaoCobranca", idImovelSituacaoCobranca).executeUpdate();
	...
	public void atualizarSituacaoCobrancaImovel(Integer idSituacaoCobrancaNova, Integer idSituacaoCobrancaBanco,
		...
		try {
			...
			consulta = "update gcom.cadastro.imovel.Imovel set "
					...
					+ "where imov_id = :idImovel and " + "cbst_id = :idSituacaoCobrancaBanco ";

			session.createQuery(consulta).setInteger("idSituacaoCobrancaNova", idSituacaoCobrancaNova)
	...
	public void atualizarNumeroMedidorEnergiaImovel(Integer matricula, String medidorEnergia)
		...
		try {
			...
			consulta += " update gcom.cadastro.imovel.Imovel " + " set imov_nnmedidorenergia = :medidorEnergia "
					+ " where imov_id = :matricula ";

			session.createQuery(consulta).setInteger("matricula", matricula).setString("medidorEnergia", medidorEnergia)
					.executeUpdate();
	 ...
	 * @date 11/05/2010
	 */
	public Collection pesquisarImovelCobrancaSituacaoPorImovel(String[] idsImovelCobrancaSituacao)
			throws ErroRepositorioException {
		...
		try {
			...
			retorno = (List) session.createQuery(hql)
					.setParameterList("idsImovelCobrancaSituacao", idsImovelCobrancaSituacao).list();
	...
	/**
	 * [UC0490] Informar Situa��o de Cobran�a [SB0004] � Selecionar Situa��es de
	 * Cobran�a
	 * 
	 * seleciona as situa��es de cobran�a (a partir da tabela COBRAN�A_SITUACAO com
	 * CBST_ICUSO=1 e CBST_ICBLOQUEIOINCLUSAO=2)retirando as ocorr�ncias com
	 * CBST_ID=CBST_ID da tabela IMOVEL_COBRANCA_SITUACAO para IMOV_ID=Id do im�vel
	...
	public Collection pesquisarCobrancaSituacao(Integer idImovel, boolean temPermissaoEspecial)
		...
		try {
			String hql = " select cbst " + " from gcom.cobranca.CobrancaSituacao as cbst "
					+ " where cbst.indicadorUso = :indicadorUso "
					+ " and cbst.indicadorBloqueioInclusao = :indicadorBloqueioInclusao " + " and cbst.id not in "
					+ "(select cbs.id " + " from gcom.cadastro.imovel.ImovelCobrancaSituacao as ics"
					+ " inner join ics.cobrancaSituacao as cbs " + " where ics.imovel.id = :idImovel "
					+ " and ics.dataRetiradaCobranca is null ) ";
			...
			retorno = (List) session.createQuery(hql).setShort("indicadorUso", ConstantesSistema.SIM)
					.setShort("indicadorBloqueioInclusao", ConstantesSistema.NAO).setInteger("idImovel", idImovel)
					.list();
	...
	public Object[] pesquisarLocalidadeCodigoSetorImovel(Integer idImovel) throws ErroRepositorioException {
		...
		try {
			...
			String sql = "SELECT l.id,sc.codigo FROM Imovel as i " + " INNER JOIN i.localidade as l "
					+ " INNER JOIN i.setorComercial as sc " + " WHERE i.id = :idImovel ";

			retorno = (Object[]) session.createQuery(sql).setInteger("idImovel", idImovel).setMaxResults(1)
					.uniqueResult();
	...
	public void atualizarClienteImovelUsuario(Integer idImovel) throws ErroRepositorioException {
		...
		try {
			...
			update += " update gcom.cadastro.cliente.ClienteImovel " + " set clim_icnomeconta = 1 "
					+ " where imov_id = :idImovel and crtp_id = " + ClienteRelacaoTipo.USUARIO
	...
	public Integer pesquisarDebitoTipoImovelDoacao(Integer idImovel) throws ErroRepositorioException {
		...
		/**
		 * * Script HQL que j� monta uma cole��o de ImovelCobrarDoacaoHelper com tudo
		 * que � necess�rio **
		 */
		try {
			String consulta = "select " + "    imovelDoacao.entidadeBeneficente.debitoTipo.id "
					+ "from gcom.cadastro.imovel.ImovelDoacao imovelDoacao " + "where"
					+ " imovelDoacao.imovel.id = :idImovel" + " AND imovelDoacao.imovel.indicadorImovelCondominio <> 1"
					+ " AND imovelDoacao.imovel.indicadorExclusao <> 1" + " AND imovelDoacao.dataCancelamento is null";

			retorno = (Integer) session.createQuery(consulta).setInteger("idImovel", idImovel).setMaxResults(1)
					.uniqueResult();
	...
	public ImovelPerfil pesquisarImovelPerfil(Integer idImovelPerfil) throws ErroRepositorioException {
		...
		try {
			String consulta = " select imovelPerfil " + " from gcom.cadastro.imovel.ImovelPerfil imovelPerfil "
					+ " where imovelPerfil.id = :idImovelPerfil ";

			retorno = (ImovelPerfil) session.createQuery(consulta).setParameter("idImovelPerfil", idImovelPerfil)
					.setMaxResults(1).uniqueResult();
			...
			if (!colecaoSubcategorias.isEmpty()) {
				...
				while (iteratorSubcategoria.hasNext()) {
					...
					imovelSubcategoriaHelper.setSubcategoria(arraySubcategorias[1] != null
							&& !arraySubcategorias[1].toString().trim().equalsIgnoreCase("")
									? (String) arraySubcategorias[1]
									: null);

					imovelSubcategoriaHelper.setCategoria(arraySubcategorias[2] != null
							&& !arraySubcategorias[2].toString().trim().equalsIgnoreCase("")
									? (String) arraySubcategorias[2]
									: null);

					imovelSubcategoriaHelper.setQuantidadeEconomias(arraySubcategorias[3] != null
							&& !arraySubcategorias[3].toString().trim().equalsIgnoreCase("")
									? ((Short) arraySubcategorias[3]).shortValue()
									: 0);
	...
	/**
	 * Caso o retorno seja TRUE, quer dizer que a altera��o da inscri��o acarretou
	 * na mudan�a do grupo de faturamento, e este novo grupo do im�vel ja foi
	 * faturado.
	 * 
	 * 
	 * @author Arthur Carvalho
	...
	public FaturamentoGrupo[] verificaInscricaoAlteradaAcarretaMudancaDoGrupoFaturamento(Integer idQuadraAnterior,
			Integer idQuadraAtual) throws ErroRepositorioException {
		...
		try {
			...
			String consultaFaturamentoGrupoOrigem = " select ftgr " + " from gcom.cadastro.localidade.Quadra qdra"
					+ " inner join qdra.rota rota" + " inner join rota.faturamentoGrupo ftgr"
					+ " where qdra.id = :idQuadraAnterior ";

			faturamentoGrupoOrigem = (FaturamentoGrupo) session.createQuery(consultaFaturamentoGrupoOrigem)
					.setParameter("idQuadraAnterior", idQuadraAnterior).setMaxResults(1).uniqueResult();

			// Consulta o faturamento grupo Destino do imovel
			String consultaFaturamentoGrupoDestino = " select ftgr " + " from gcom.cadastro.localidade.Quadra qdra"
					+ " inner join qdra.rota rota" + " inner join rota.faturamentoGrupo ftgr"
					+ " where qdra.id = :idQuadraAtual ";

			faturamentoGrupoDestino = (FaturamentoGrupo) session.createQuery(consultaFaturamentoGrupoDestino)
					.setParameter("idQuadraAtual", idQuadraAtual).setMaxResults(1).uniqueResult();
	 ...
	 * @date 21/09/2010
	 * 
	 *       UC_0009 - Manter Cliente [FS0008] ? Verificar permiss�o especial para
	 *       cliente de im�vel p�blico
	 * 
	 *       Verifica se o Cliente possui algum im�vel, cujo o tipo da categoria em
	 *       subcategoria seja igual a PUBLICO.
	 * 
	 *       Caso o cliente possua algum im�vel, retornar a quantidade de imoveis
	 *       nesta situa��o Caso contr�rio retorna zero.
	 * 
	 * @param idCliente
	 * @return Integer
	 * 
	...
	public Integer obterQuantidadeImoveisPublicosAssociadoAoCliente(Integer idCliente) throws ErroRepositorioException {
		...
		try {
			...
			consulta = " select count (ci.id) " + " from ClienteImovel ci " + " inner join ci.imovel imo "
					+ " inner join imo.imovelSubcategorias imoSub " + " inner join imoSub.comp_id.subcategoria sbCat "
					+ " inner join sbCat.categoria cat " + " where ci.cliente = :idCliente "
					+ " and cat.categoriaTipo = :tipoCategoria ";

			retorno = (Integer) session.createQuery(consulta).setParameter("idCliente", idCliente)
					.setParameter("tipoCategoria", CategoriaTipo.PUBLICO).setMaxResults(1).uniqueResult();
	...
	public boolean verificaGeracaoDadosLeituraGrupoFaturamento(FaturamentoGrupo faturamentoGrupo)
		...
		try {
			// Consulta o faturamento grupo Origem
			String consulta = " select mvre " + " from gcom.micromedicao.MovimentoRoteiroEmpresa mvre"
	...
	public ImovelInscricaoAlterada verificarDuplicidadeImovelInscricaoAlterada(Integer idLocalidade,
			Integer idSetorComercial, Integer idQuadra, Integer idQuadraFace, Integer lote, Integer subLote)
			throws ErroRepositorioException {
		...
		try {
			// Consulta o faturamento grupo Origem
			String consulta = " select imia " + " from gcom.cadastro.imovel.ImovelInscricaoAlterada imia"
					+ " where imia.indicadorAtualizado = :indicadorAtualizado "
					+ " and imia.indicadorAtualizacaoExcluida = :indicadorAtualizacaoExcluida"
					+ " and (imia.indicadorErroAlteracao is null)" + " and imia.localidadeAtual.id = :idLocalidade"
					+ " and imia.setorComercialAtual.id = :idSetorComercial" + " and imia.quadraAtual.id = :idQuadra"
					+ " and imia.loteAtual = :lote" + " and imia.subLoteAtual = :subLote";
			...
			if (idQuadraFace != null) {
				imovelInscricaoAlterada = (ImovelInscricaoAlterada) session.createQuery(consulta)
						...
						.setParameter("idLocalidade", idLocalidade).setParameter("idSetorComercial", idSetorComercial)
						.setParameter("idQuadra", idQuadra).setParameter("lote", lote).setParameter("subLote", subLote)
						.setParameter("idQuadraFaceAtual", idQuadraFace).setMaxResults(1).uniqueResult();
			} else {
				imovelInscricaoAlterada = (ImovelInscricaoAlterada) session.createQuery(consulta)
						.setParameter("indicadorAtualizado", ConstantesSistema.NAO)
						.setParameter("indicadorAtualizacaoExcluida", ConstantesSistema.NAO)
	...
	public Municipio pesquisarMunicipioImovel(Integer idImovel) throws ErroRepositorioException {
		...
		try {
			...
			String consulta = " select municipio " + " from gcom.cadastro.imovel.Imovel imovel"
					+ " Inner Join imovel.logradouroBairro logradouroBairro"
					+ " Inner Join logradouroBairro.logradouro logradouro"
					+ " Inner Join logradouro.municipio municipio" + " where imovel.id = :idImovel";

			retorno = (Municipio) session.createQuery(consulta).setParameter("idImovel", idImovel).setMaxResults(1)
					.uniqueResult();
	...
	public Short consultarNumeroReparcelamentoConsecutivosImovel(Integer idImovel) throws ErroRepositorioException {
		...
		try {
			...
			consulta = "SELECT imovel.numeroReparcelamentoConsecutivos " + "FROM Imovel imovel "
					+ "where imovel.id = :idImovel ";

			retorno = (Short) session.createQuery(consulta).setInteger("idImovel", idImovel.intValue()).setMaxResults(1)
					.uniqueResult();
	...
	/**
	 * autor: Adriana Muniz Data: 12/05/2011 Pesquisa o id da quadra pelo id do
	 * im�vel - Gerar Arquivo da declara��o de quita��o anual de d�bito
	 */
	public Integer pesquisaIdQuadraImovel(Integer idImovel) throws ErroRepositorioException {
		...
		try {
			consulta = "select i.quadra.id " + " from Imovel i " + " where i.id = :idImovel ";
	...
	public boolean verificaGeracaoDadosLeituraRota(FaturamentoGrupo faturamentoGrupo, Rota rota)
		...
		try {
			// Consulta o faturamento grupo Origem
			String consulta = " select mvre " + " from gcom.micromedicao.MovimentoRoteiroEmpresa mvre"
					+ " where mvre.faturamentoGrupo.id = :idFaturamentoGrupo " + " and mvre.rota.id = :idRota "
					+ " and mvre.anoMesMovimento = :anoMesReferenciaFaturamentoGrupo";

			movimentoRoteiroEmpresa = (MovimentoRoteiroEmpresa) session.createQuery(consulta)
					.setParameter("idFaturamentoGrupo", faturamentoGrupo.getId()).setParameter("idRota", rota.getId())
					.setParameter("anoMesReferenciaFaturamentoGrupo", faturamentoGrupo.getAnoMesReferencia())
					.setMaxResults(1).uniqueResult();
	...
	public ImovelPerfil pesquisarImovelPerfilIdImovel(Integer idImovel) throws ErroRepositorioException {
		...
		try {
			...
			String consulta = " SELECT ip " + " FROM gcom.cadastro.imovel.Imovel i " + " INNER JOIN i.imovelPerfil ip "
					+ " WHERE i.id = :idImovel ";
	...
	public Collection<Imovel> consultarImovelLocalidade(Integer idLocalidade) throws ErroRepositorioException {
		...
		try {
			consulta = "SELECT imov.imov_id AS idImov, loca.loca_nnconsumograndeusuario AS consumo, "
					+ " imov.iper_id AS perfil" + " FROM cadastro.imovel imov "
					+ " INNER JOIN cadastro.localidade loca ON loca.loca_id = :idLocalidade "
					+ " AND imov.loca_id = loca.loca_id ";

			Collection colecao = (Collection) session.createSQLQuery(consulta).addScalar("idImov", Hibernate.INTEGER)
					.addScalar("consumo", Hibernate.INTEGER).addScalar("perfil", Hibernate.INTEGER)
	...
	public void atualizarImovelPerfil(Integer idImovel, Integer idPerfil) throws ErroRepositorioException {
		...
		try {
			consulta = " UPDATE gcom.cadastro.imovel.Imovel " + " SET iper_id = :idPerfil "
					+ " WHERE imov_id = :idImovel ";

			session.createQuery(consulta).setInteger("idPerfil", idPerfil).setInteger("idImovel", idImovel)
					.executeUpdate();
	...
	public Collection consultarImovel(Integer idLocalidade, Integer idGerenciaRegional, Integer idUnidadeNegocio)
		...
		try {
			...
			String sql = "SELECT imov " + " FROM Imovel as imov " + " INNER JOIN fetch imov.localidade as loca ";
	...
	public Collection consultarImovelCadastro(Integer idLocalidade, Integer idGerenciaRegional,
		...
		try {
			...
			String sql = "SELECT distinct(imov) " + " FROM TarifaSocialDadoEconomia tsde "
					+ " INNER JOIN tsde.imovel as imov " + " INNER JOIN fetch imov.localidade as loca ";
			...
			if (anoMesPesquisaInicial != null) {
				...
				sqlParte2 = sqlParte2 + " and  tsde.dataImplantacao >= to_date('" + Util.formatarDataComTracoAAAAMMDD(
						Util.gerarDataInicialApartirAnoMesRefencia(anoMesPesquisaInicial)) + "','YYYY-MM-DD') ";
...
//				parameters.put("anoMesPesquisaFinal", Util.gerarDataApartirAnoMesRefencia(anoMesPesquisaFinal));
	...
	public Collection pesquisarImovelEconomia(Integer idImovel) throws ErroRepositorioException {
		...
		try {
			...
			consulta = " select imec " + " FROM TarifaSocialDadoEconomia tsde "
					+ " inner join tsde.imovelEconomia imec " + " inner join tsde.imovel imov "
					+ " where imov.id = :idImovel " + " and imec.numeroCelpe is not null ";
	...
	public TarifaSocialDadoEconomia pesquisarTarifaSocialDadoEconomia(Integer idImovel)
		...
		try {
			...
			consulta = " select tsde " + " FROM TarifaSocialDadoEconomia tsde "
					+ " inner join fetch tsde.tarifaSocialCartaoTipo " + " inner join tsde.imovel imov "
					+ " where imov.id = :idImovel ";

			retorno = (TarifaSocialDadoEconomia) session.createQuery(consulta).setInteger("idImovel", idImovel)
					.setMaxResults(1).uniqueResult();
	...
	public TarifaSocialCarta pesquisarTarifaSocialCarta(Integer idImovel, Integer codigoTipoCarta)
		...
		try {
			...
			consulta = " select tscr " + " FROM TarifaSocialCarta tscr " + " inner join tscr.imovel imov "
					+ " inner join fetch tscr.tarifaSocialComandoCarta tscc " + " where imov.id = :idImovel "
					+ " and tscc.dataExecucao is null " + " and tscc.dataGeracao is not null "
					+ " and tscc.codigoTipoCarta = :codigoTipoCarta ";

			retorno = (TarifaSocialCarta) session.createQuery(consulta).setInteger("idImovel", idImovel)
					.setInteger("codigoTipoCarta", codigoTipoCarta).setMaxResults(1).uniqueResult();
	...
	public Integer pesquisarQuantidadeImoveisTarifaSocialCarta(Integer idTarifaSocialComandoCarta)
		...
		try {
			...
			consulta = " select count(imov.id) " + " FROM TarifaSocialCarta tscr " + " inner join tscr.imovel imov "
					+ " inner join tscr.tarifaSocialComandoCarta tscc "
					+ " where tscc.id = :idTarifaSocialComandoCarta ";

			retorno = (Integer) session.createQuery(consulta)
					.setInteger("idTarifaSocialComandoCarta", idTarifaSocialComandoCarta).setMaxResults(1)
					.uniqueResult();
	...
	public void atualizarTarifaSocialComandoCarta(Integer idTarifaSocialComandoCarta, Integer qtdeImoveis)
		...
		try {
			consulta = " UPDATE gcom.cadastro.tarifasocial.TarifaSocialComandoCarta "
					+ " SET tscc_qtcartasgeradas = :qtdeImoveis, " + " tscc_dtprocessamento = :dataAtual "
					+ " WHERE tscc_id = :idTarifaSocialComandoCarta ";

			session.createQuery(consulta).setInteger("qtdeImoveis", qtdeImoveis)
	...
	public Collection pesquisarTarifaSocialCarta(Integer idTarifaSocialComandoCarta) throws ErroRepositorioException {
		...
		try {
			...
			consulta = " select tscr " + " FROM TarifaSocialCarta tscr " + " inner join fetch tscr.cliente clie "
					+ " inner join tscr.tarifaSocialComandoCarta tscc " + " inner join tscr.imovel imov "
					+ " inner join imov.quadra quadra " + " inner join quadra.rota rota "
					+ " inner join rota.faturamentoGrupo ftgr " + " where tscc.id = :idTarifaSocialComandoCarta "
					+ " order by ftgr.id,imov.localidade.id,imov.setorComercial.codigo,quadra.numeroQuadra,imov.lote,imov.subLote ";

			retorno = (Collection) session.createQuery(consulta)
					.setInteger("idTarifaSocialComandoCarta", idTarifaSocialComandoCarta).list();
	...
	public BigDecimal pesquisarValorContaTarifaSocialCartaDebito(Integer idTarifaSocialComandoCarta, Integer idImovel)
		...
		try {
			...
			consulta = " select sum(tscd.valorConta) " + " FROM TarifaSocialCartaDebito tscd"
					+ " inner join tscd.imovel imov " + " inner join tscd.tarifaSocialComandoCarta tscc "
					+ " where tscc.id = :idTarifaSocialComandoCarta and " + " imov.id = :idImovel ";
	...
	public Collection pesquisarTarifaSocialComandoCarta(Integer codigoTipoCarta, String situacao, Integer numeroPagina)
		...
		try {
			...
			consulta = " select tscc " + " FROM TarifaSocialComandoCarta tscc " + " where  ";
			...
			if (numeroPagina.intValue() == -1) {
				...
				if (!codigoTipoCarta.equals(new Integer(3))) {
					...
					retorno = (Collection) session.createQuery(consulta).setInteger("codigoTipoCarta", codigoTipoCarta)
							.list();
				...
				if (!codigoTipoCarta.equals(new Integer(3))) {
					...
					retorno = (Collection) session.createQuery(consulta).setFirstResult(10 * numeroPagina)
	...
	public Integer pesquisarQtdeTarifaSocialComandoCarta(Integer codigoTipoCarta, String situacao)
		...
		try {
			...
			consulta = " select count(tscc.id) " + " FROM TarifaSocialComandoCarta tscc " + " where  ";
			...
			if (!codigoTipoCarta.equals(new Integer(3))) {
				...
				retorno = (Integer) session.createQuery(consulta).setInteger("codigoTipoCarta", codigoTipoCarta)
						.setMaxResults(1).uniqueResult();
	...
	public void removerComando(Integer idTarifaSocialComandoCarta) throws ErroRepositorioException {
		...
		try {
			...
			session.createQuery(remocaoTarifaSocialCartaDebito)
					.setInteger("idTarifaSocialComandoCarta", idTarifaSocialComandoCarta).executeUpdate();
	...
	public Collection pesquisarImoveisTarifaSocialCarta(Integer idTarifaSocialComandoCarta, Integer codigoTipoCarta)
		...
		try {
			...
			consulta = " select imov " + " FROM TarifaSocialCarta tscr " + " inner join tscr.imovel imov "
					+ " inner join tscr.tarifaSocialComandoCarta tscc "
					+ " where tscc.id = :idTarifaSocialComandoCarta ";
	...
	public Collection pesquisarContasTarifaSocialCartaDebito(Integer idTarifaSocialComandoCarta, Integer idImovel)
		...
		try {
			...
			consulta = " select tscd.conta.id " + " FROM TarifaSocialCartaDebito tscd" + " inner join tscd.imovel imov "
	...
	/**
	 * [UC1160] Processar Comando Gerado Carta Tarifa Social [SB0004]�Retirar Im�vel
	 * tarifa Social
	 * 
	 * @author Vivianne Sousa
	 * @date 04/04/2011
	...
	public void retirarImovelTarifaSocial(Integer motivoExclusao, Integer idImovel) throws ErroRepositorioException {
		...
		try {
			consulta = " UPDATE gcom.cadastro.tarifasocial.TarifaSocialDadoEconomia "
					+ " SET tsde_dtexclusao = :dataExclusao, " + " etsm_id = :motivoExclusao, "
					+ " tsde_dtrevisao = null, " + " rtsm_id = null " + " WHERE imov_id = :idImovel ";

			session.createQuery(consulta).setInteger("motivoExclusao", motivoExclusao).setInteger("idImovel", idImovel)
...
//				.executeUpdate();
			...
			session.createQuery(consulta).setShort("indicadorExcluidoTarifaSocial", ConstantesSistema.SIM)
	...
	public void atualizarDataExecucaoTarifaSocialComandoCarta(Integer idTarifaSocialComandoCarta)
		...
		try {
			consulta = " UPDATE gcom.cadastro.tarifasocial.TarifaSocialComandoCarta "
					+ " SET tscc_dtexecucao = :dataExecucao " + " WHERE tscc_id = :idTarifaSocialComandoCarta ";

			session.createQuery(consulta).setInteger("idTarifaSocialComandoCarta", idTarifaSocialComandoCarta)
	...
	public Collection pesquisarImoveisTarifaSocial(Integer idLocalidade) throws ErroRepositorioException {
		...
		try {
			...
			consulta = "SELECT imov " + " FROM Imovel as imov " + " INNER JOIN imov.localidade as loca "
					+ " INNER JOIN imov.ligacaoAgua as lagu " + " WHERE imov.imovelPerfil.id = :imovelPerfil "
					+ " AND loca.id = :idLocalidade " + " AND lagu.hidrometroInstalacaoHistorico is not null "
					+ " AND lagu.id = imov.id "
					+ " AND ( imov.indicadorExclusao IS NULL or imov.indicadorExclusao != :indicadorExclusao ) ";

			retorno = (Collection) session.createQuery(consulta).setInteger("idLocalidade", idLocalidade)
					.setInteger("imovelPerfil", ImovelPerfil.TARIFA_SOCIAL)
					.setShort("indicadorExclusao", ConstantesSistema.SIM).list();
	...
	public Collection pesquisarQtdeImoveisExcluidostarifaSocial(Integer codigoTipoCarta, Integer idGerencia,
		...
		try {
			...
			consulta = " select count(tscr.imovel.id)," + // 0
					...
					" FROM TarifaSocialCarta tscr " + " inner join tscr.tarifaSocialComandoCarta tscc "
					+ " where tscc.codigoTipoCarta = :codigoTipoCarta " + " and  tscc.dataExecucao >= to_date('"
					+ Util.formatarDataComTracoAAAAMMDD(Util.gerarDataInicialApartirAnoMesRefencia(referenciaInicial))
					+ "','YYYY-MM-DD') " + " and  tscc.dataExecucao <= to_date('"
...
//			}
	...
	public Integer pesquisarQtdeImoveisExcluidostarifaSocial(Integer referenciaInicial, Integer refereciaFinal,
		...
		try {
			...
			consulta = " select count(tscr.imovel.id)" + " FROM TarifaSocialCarta tscr "
					+ " inner join tscr.tarifaSocialComandoCarta tscc "
					+ " where tscc.codigoTipoCarta = :codigoTipoCarta "
					+ " and tscr.indicadorExcluidoTarifaSocial = :indicadorExcluidoTarifaSocial "
					+ " and tscr.localidade.id = :idLocalidade " + " and tscr.codigoMotivo = :motivo " +
...
//					.setDate("refereciaFinal", Util.gerarDataApartirAnoMesRefencia(refereciaFinal))
	...
	public String pesquisarDescricaoMotivoCarta(Integer idCodigoMotivo) throws ErroRepositorioException {
		...
		try {
			...
			consulta = " select tsmc.descricaoMotivoCarta " + " FROM TarifaSocialMotivoCarta tsmc "
					+ " where tsmc.id = :idCodigoMotivo ";

			retorno = (String) session.createQuery(consulta).setInteger("idCodigoMotivo", idCodigoMotivo)
					.setMaxResults(1).uniqueResult();
	...
	public Collection pesquisarQtdeTarifaSocialDadoEconomia(Integer idtarifaSocialExclusaoMotivo,
		...
		try {
			...
			consulta = " select count(distinct imov.id), " + // 0
					...
					" uneg.nome," + // 5
					" loca.descricao " + // 6
					" FROM TarifaSocialDadoEconomia tsde " + " inner join tsde.imovel imov "
					+ " left join imov.localidade loca " + " left join loca.gerenciaRegional greg "
...
//					" and tsde.dataExclusao <= :refereciaFinal" ;
					...
					+ Util.formatarDataComTracoAAAAMMDD(Util.gerarDataInicialApartirAnoMesRefencia(referenciaInicial))
					+ "','YYYY-MM-DD') " + " and  tsde.dataExclusao <= to_date('"
					+ Util.formatarDataComTracoAAAAMMDD(Util.gerarDataApartirAnoMesRefencia(refereciaFinal))
					+ "','YYYY-MM-DD') ";
...
//		parameters.put("refereciaFinal", Util.gerarDataApartirAnoMesRefencia(refereciaFinal));
			...
			consulta = consulta + " group by greg.id, uneg.id, loca.id, greg.nome, uneg.nome, loca.descricao "
	 ...
	 * @throws ControladorException
	 */
	public void atualizarDadosFaturamentoSituacaoHistorico(Integer idImovel, Integer referenciaFaturamentoGrupo,
			String observacaoRetira) throws ErroRepositorioException {
		...
		try {
			consulta = " UPDATE gcom.faturamento.FaturamentoSituacaoHistorico "
					...
					+ " ftsh_dsobservacaoretira = :observacaoRetira , " + " usur_idretira = :idUsuario "
					+ " WHERE imov_id = :idImovel and " + " ftsh_amfaturamentoretirada is null ";

			session.createQuery(consulta).setInteger("idImovel", idImovel).setTimestamp("ultimaAlteracao", new Date())
					.setInteger("idUsuario", (Usuario.USUARIO_BATCH).getId())
					.setInteger("referenciaFaturamentoGrupo", referenciaFaturamentoGrupo)
					.setString("observacaoRetira", observacaoRetira).executeUpdate();
	...
	public void atualizarDadosImovel(Integer idImovel, Integer referencia) throws ErroRepositorioException {
		...
		try {
			consulta = " UPDATE gcom.cadastro.imovel.Imovel " + " SET iper_id = :imovelPerfil , " + " ftst_id = null , "
					+ " cstf_id = :consumoTarifa, " + " imov_amrefexclusaotarifasocial = :referencia"
					+ " WHERE imov_id = :idImovel ";

			session.createQuery(consulta).setInteger("idImovel", idImovel)
					.setInteger("imovelPerfil", ImovelPerfil.NORMAL)
					.setInteger("consumoTarifa", ConsumoTarifa.CONSUMO_NORMAL).setInteger("referencia", referencia)
					.executeUpdate();
	...
	public void removerCartasComando(Integer idTarifaSocialComandoCarta, Integer idLocalidade, Integer tipoCarta)
		...
		try {
			...
			if (tipoCarta.equals(new Integer("2"))) {
				String consulta = " select imov.id " + " FROM TarifaSocialCarta tscr " + " inner join tscr.imovel imov "
				...
				if (colecaoIdsImovel != null && !colecaoIdsImovel.isEmpty()) {
					String remocaoTarifaSocialCartaDebito = "delete TarifaSocialCartaDebito "
							+ "where tscc_id = :idTarifaSocialComandoCarta and " + "imov_id in (:colecaoIdsImovel) ";
					session.createQuery(remocaoTarifaSocialCartaDebito)
			...
			String remocaoTarifaSocialCarta = "delete TarifaSocialCarta "
					+ "where tscc_id = :idTarifaSocialComandoCarta " + " and loca_id = :idLocalidade ";
			session.createQuery(remocaoTarifaSocialCarta)
					.setInteger("idTarifaSocialComandoCarta", idTarifaSocialComandoCarta)
	...
	public Integer pesquisarAnoMesExclusaoTarifaSocialImovel(Integer idImovel) throws ErroRepositorioException {
		...
		try {
			...
			consulta = " select imov.anoMesExclusaoTarifaSocial " + " FROM Imovel imov "
					+ " where imov.id = :idImovel ";
	...
	public void retirarCobrancaImovelCobrancaPorEmpresa(Integer idImovel, Integer idCobrancaSituacao, Usuario usuario)
		...
		try {
			...
			consulta.append(
					" update Imovel set cbst_id = null, imov_tmultimaalteracao = :ultimaAlteracao where imov_id = :idImovel ");

			session.createQuery(consulta.toString()).setTimestamp("ultimaAlteracao", new Date())
					.setInteger("idImovel", idImovel).executeUpdate();

			consulta = new StringBuilder();
			consulta.append(
					" update ImovelCobrancaSituacao  set iscb_dtretiradacobranca = :data , iscb_tmultimaalteracao = :ultimaAlteracao ")
					.append(" where imov_id = :imovel and cbst_id = :idCobrancaSituacao and iscb_dtretiradacobranca is null ");

			session.createQuery(consulta.toString()).setTimestamp("ultimaAlteracao", new Date())
					.setDate("data", new Date()).setInteger("imovel", idImovel)
					.setInteger("idCobrancaSituacao", idCobrancaSituacao).executeUpdate();
			...
			session.createQuery(consulta.toString()).setTimestamp("ultimaAlteracao", new Date())
					.setInteger("retirada", Util.formataAnoMes(new Date())).setInteger("imovel", idImovel)
					.setInteger("idUsuario", usuario.getId())
					.setInteger("idCobrancaSituacaoTipo", CobrancaSituacaoTipo.COBRANCA_EMPRESA_TERCEIRIZADA)
					.setString("observacao", "FIM DO CICLO DE COBRAN�A POR EMPRESA.").executeUpdate();
	...
	public Collection<Integer[]> pesquisarIdsImoveis(String[] idsOrdemServico) throws ErroRepositorioException {
		...
		try {
			consulta = " select orse.orse_id as idOS, orse.imov_id as idImovel "
					+ " from atendimentopublico.ordem_servico orse " + " where orse.orse_id in( ";
			...
			retorno = session.createSQLQuery(consulta).addScalar("idOS", Hibernate.INTEGER)
	...
	/**
	 * [UC0000] Obter Consumo N�o Medido por Par�metro [FS0001] - Verificar "Pontos
	 * de Utiliza��o" N�o Informado.
	 * 
	 * @author Mariana Victor
	 * @date 23/05/2011
	 * 
	...
	public Short pesquisarPontosUtilizacaoImovel(Integer idImovel) throws ErroRepositorioException {
		...
		try {
			...
			consulta = " select imov.imov_nnpontosutilizacao as pontosUtilizacao " + " FROM cadastro.imovel imov "
					+ " where imov.imov_id = :idImovel ";

			retorno = (Short) session.createSQLQuery(consulta).addScalar("pontosUtilizacao", Hibernate.SHORT)
					.setInteger("idImovel", idImovel).setMaxResults(1).uniqueResult();
	...
	/**
	 * [UC0000] Obter Consumo N�o Medido por Par�metro [FS0002] - Verificar �N�mero
	 * de Moradores� N�o Informado.
	 * 
	 * @author Mariana Victor
	 * @date 23/05/2011
	 * 
	...
	public Short pesquisarNumeroMoradoresImovel(Integer idImovel) throws ErroRepositorioException {
		...
		try {
			...
			consulta = " select imov.imov_nnmorador as numeroMoradores " + " FROM cadastro.imovel imov "
					+ " where imov.imov_id = :idImovel ";

			retorno = (Short) session.createSQLQuery(consulta).addScalar("numeroMoradores", Hibernate.SHORT)
					.setInteger("idImovel", idImovel).setMaxResults(1).uniqueResult();
	...
	public Integer pesquisarQuantidadeImoveisDoacoes(GerarRelatorioImoveisDoacoesHelper filtro)
		...
		try {
			consulta = "SELECT COUNT(*) FROM gcom.cadastro.imovel.ImovelDoacao imov "
					+ "LEFT JOIN imov.usuarioCancelamento usurCancel " + "INNER JOIN imov.usuarioAdesao usuAdesao "
					+ "WHERE 1=1 ";
			...
			// Identificador d Usu�rio de Ades�o
			...
			// Identificador do Usu�rio de Cancelmaneto
			...
			// Datas do per�odo de cancelamento
			...
			// Ano/Mes referencia fim da doa��o
	 ...
	 * @data 13/06/2011
	 */
	public Collection pesquisarImoveisDoacoes(GerarRelatorioImoveisDoacoesHelper filtro)
			throws ErroRepositorioException {
		Collection imoveis;
		...
		try {
			...
			consulta = "SELECT " + "imovelDoacao.entidadeBeneficente.cliente.nome, " + "imovel.id, "
					+ "imovelDoacao.dataAdesao, " + "imovelDoacao.dataCancelamento, " + "usuAdesao.nomeUsuario, "
					+ "usurCancel.nomeUsuario, " + "imovelDoacao.anoMesReferenciaInicial, "
					+ "imovelDoacao.anoMesReferenciaFinal, " + "imovelDoacao.valorDoacao, " + "clienteUsuario.nome "
					+ "FROM gcom.cadastro.imovel.ImovelDoacao imovelDoacao "
					+ "LEFT JOIN imovelDoacao.usuarioCancelamento usurCancel "
					+ "INNER JOIN imovelDoacao.usuarioAdesao usuAdesao " + "INNER JOIN imovelDoacao.imovel imovel "
					+ "INNER JOIN imovel.clienteImoveis clienteImoveis WITH (clienteImoveis.clienteRelacaoTipo.id = :relacaoTipo) "
					+ "and clienteImoveis.dataFimRelacao is null " + "INNER JOIN clienteImoveis.cliente clienteUsuario "
					+ "WHERE 1=1 ";
	 ...
	 * @date 09/08/2011
	 */
	public void atualizarSituacaoEspecialFaturamentoImovel(Integer idImovel, Integer idFaturamentoSituacaoTipo,
		...
		try {
			consulta = "update gcom.cadastro.imovel.Imovel set " + "ftst_id = :idFaturamentoSituacaoTipo,"
					+ "ftsm_id = :idFaturamentoSituacaoMotivo," + "imov_tmultimaalteracao = :ultimaAlteracao "
					+ "where imov_id = :idImovel";
	 ...
	 * 
	 * [UC0098] Manter V�nculos de Im�veis para Rateio de Consumo [SB0001] Atualizar
	 * Tipo de Rateio
	 * 
	...
	public Integer pesquisarImovelAreaComum(Integer idImovelCondominio) throws ErroRepositorioException {
		...
		try {
			...
			consulta = "SELECT imov.id " + "FROM Imovel imov " // imovel
			...
			retorno = (Integer) session.createQuery(consulta).setInteger("idImovelCondominio", idImovelCondominio)
					.setShort("indicadorImovelAreaComum", ConstantesSistema.SIM).uniqueResult();
	 ...
	 * </p>
	 * <p>
	 * [SB0001] - [FS0012] - Caso a matr�cula do im�vel para �rea comum informada
	 * n�o exista na tabela IMOVEL, exibir a mensagem "Matr�cula inexistente no
	...
	public Short verificarExistenciaDoImovel(Integer idImovel) throws ErroRepositorioException {
		...
		try {
			consulta = "SELECT imovel.indicadorExclusao " + "FROM Imovel imovel " + "WHERE imovel.id = :idImovel";

			retorno = (Short) session.createQuery(consulta).setInteger("idImovel", idImovel).setMaxResults(1)
					.uniqueResult();
	...
	public Integer pesquisarImovelCondominio(Integer idImovel) throws ErroRepositorioException {
		...
		try {
			...
			consulta = "SELECT imov.imovelCondominio.id " + "FROM Imovel imov " + "WHERE imov.id = :idImovel";
	...
	public void atualizarIndicadorImovelAreaComumDoImovel(Integer idImovel, Short indicadorImovelAreaComum)
		...
		try {
			consulta = "UPDATE gcom.cadastro.imovel.Imovel SET "
					...
					+ "imov_tmultimaalteracao = :ultimaAlteracao " + "WHERE imov_id = :idImovel";
	...
	public Collection<ImovelRamoAtividade> pesquisarRamoAtividadeDoImovel(Integer idImovel)
		...
		try {
			consulta = "from ImovelRamoAtividade ramoAtividade " + "where ramoAtividade.comp_id.imovel.id = :idImovel";

			colecaoRamoAtividadeImovel = session.createQuery(consulta).setInteger("idImovel", idImovel).list();
	...
	public Collection<Integer> pesquisarIdImoveisAprovados() throws ErroRepositorioException {
		Session session = HibernateUtil.getSession();
		try {
			String consulta = "SELECT imovel.id " + "FROM ImovelControleAtualizacaoCadastral icac "
					+ "INNER JOIN icac.imovel imovel " + "INNER JOIN icac.situacaoAtualizacaoCadastral situacao "
	...
	public LogradouroTipo pesquisarTipoLogradouro(Integer idTipoLogradouro) throws ErroRepositorioException {
		Session session = HibernateUtil.getSession();
		try {
			String consulta = "SELECT tipo " + "FROM LogradouroTipo tipo " + "WHERE tipo.id = :idTipo ";

			return (LogradouroTipo) session.createQuery(consulta).setInteger("idTipo", idTipoLogradouro).uniqueResult();
		} catch (HibernateException e) {
			throw new ErroRepositorioException("Erro no Hibernate");
		} finally {
			HibernateUtil.closeSession(session);
	...
	public Logradouro pesquisarLogradouro(Integer codigoLogradouro) throws ErroRepositorioException {
		Session session = HibernateUtil.getSession();
		try {
			String consulta = "SELECT logr " + "FROM Logradouro logr " + "WHERE logr.id = :codigo ";

			return (Logradouro) session.createQuery(consulta).setInteger("codigo", codigoLogradouro).uniqueResult();
		} catch (HibernateException e) {
			throw new ErroRepositorioException("Erro no Hibernate");
		} finally {
			HibernateUtil.closeSession(session);
	...
	public Integer pesquisarLogradouroImovelAtualizacaoCadastral(Integer matriculaImovel)
			throws ErroRepositorioException {
		Session session = HibernateUtil.getSession();
		try {
			String consulta = " SELECT imovel.idLogradouro " + " FROM ImovelAtualizacaoCadastral imovel "
	...
	public ImovelCobrancaSituacao obterImovelCobrancaSituacao(Integer idImovelSituacaoCobranca)
		...
		try {
			...
			retorno = (ImovelCobrancaSituacao) session.createQuery(consulta)
					.setInteger("idImovelSituacaoCobranca", idImovelSituacaoCobranca).setMaxResults(1).uniqueResult();
		} catch (HibernateException e) {
			throw new ErroRepositorioException(e, "Erro no Hibernate");
		} finally {
			HibernateUtil.closeSession(session);
	...
	public DebitoAutomatico pesquisarDebitoAutomaticoAtivoImovel(Integer idImovel) throws ErroRepositorioException {
		...
		try {
			consulta.append("SELECT debitoAutomatico ").append("from DebitoAutomatico debitoAutomatico ")
					.append("where debitoAutomatico.imovel.id = :idImovel ")
					.append("and debitoAutomatico.dataExclusao is null ");

			retorno = (DebitoAutomatico) session.createQuery(consulta.toString())
	...
	public void incluirImovelCobranca(Integer idCobrancaSituacao, Integer idCobrancaSituacaoTipo, Integer idImovel)
		...
		try {
			consulta.append(" update gcom.cadastro.imovel.Imovel set ").append(" cbst_id = :idCobrancaSituacao, ")
					.append(" cbsp_id = :idCobrancaSituacaoTipo, ")
					.append(" imov_tmultimaalteracao = :ultimaAlteracao  ").append(" where imov_id = :idImovel ");

			session.createQuery(consulta.toString()).setInteger("idCobrancaSituacao", idCobrancaSituacao)
	...
	public List<Imovel> pesquisarCondominios(Rota rota) throws ErroRepositorioException {
		...
		try {
			consulta.append(" select imovel from Imovel imovel ").append(" inner join imovel.quadra quadra ")
			...
			return (List<Imovel>) session.createQuery(consulta.toString()).setInteger("idRota", rota.getId())
	...
	public List<Imovel> pesquisarImoveisBolsaAgua(Rota rota) throws ErroRepositorioException {
		...
		try {
			consulta.append(" select imovel from Imovel imovel ").append(" inner join fetch imovel.quadra quadra ")
					...
					.append(" and rota.id = :idRota ").append(" and subcategoria.categoria.id = :idCategoria ")
	...
	public Collection pesquisarImoveisBolsaAguaPorClienteId(Integer idCliente) throws ErroRepositorioException {
		...
		try {
			consulta = "select i.imov_id as imovel from cadastro.cliente_imovel ci "
					+ " inner join cadastro.imovel i on i.imov_id = ci.imov_id " + " where iper_id = "
					+ ImovelPerfil.BOLSA_AGUA + " and i.last_id = " + LigacaoAguaSituacao.LIGADO
					+ " and (i.ftst_id is null or i.ftst_id <> " + FaturamentoSituacaoTipo.PARALISAR_EMISSAO_CONTAS
					+ ")" + " and ci.clie_id = " + idCliente;

			return session.createSQLQuery(consulta).addScalar("imovel", Hibernate.INTEGER).list();
	...
	public Imovel consultarImovelBolsaAgua(Integer idImovel) throws ErroRepositorioException {
		...
		try {
			consulta.append(" select imovel from Imovel imovel ").append(" inner join fetch imovel.quadra quadra ")
					...
					.append(" where imovel.imovelPerfil.id = :perfilBolsaAgua  ")
					.append(" and (imovel.ligacaoAguaSituacao.id = :ligado or imovel.ligacaoEsgotoSituacao.id = :ligado)")
					.append(" and imovel.id = :idImovel ").append(" and subcategoria.categoria.id = :idCategoria ")
					.append(" and (imovel.faturamentoSituacaoTipo.id is null or imovel.faturamentoSituacaoTipo.id <> :idFaturamentoSituacaoTipo)");
	...
	public Conta consultarContaBolsaAgua(Integer idConta) throws ErroRepositorioException {
		...
		try {
			consulta.append(" select conta from Conta conta ")
					...
					.append(" where conta.imovelPerfil.id = :perfilBolsaAgua  ").append(" and conta.id = :idConta ");

			return (Conta) session.createQuery(consulta.toString())
					.setInteger("perfilBolsaAgua", ImovelPerfil.BOLSA_AGUA).setInteger("idConta", idConta)
					.uniqueResult();
	...
	public List<Integer> pesquisarImovelBolsaAguaPorRota(Integer idRota) throws ErroRepositorioException {
		...
		try {
			consulta = " SELECT i.imov_id as idImovel FROM cadastro.imovel i  "
					...
					+ " AND i.iper_id = :perfilBolsaAgua " + " AND r.rota_id = :idRota";

			idsImoveis = (List<Integer>) session.createSQLQuery(consulta).addScalar("idImovel", Hibernate.INTEGER)
					.setInteger("perfilBolsaAgua", ImovelPerfil.BOLSA_AGUA).setInteger("idRota", idRota).list();
	...
	public List<Integer> pesquisarImovelElegivelBolsaAguaPorRota(Integer idRota) throws ErroRepositorioException {
		...
		try {
			consulta.append(" Select distinct i.imov_id as idImovel from cadastro.imovel_subcategoria scat   ")
					...
					.append("      where i.iper_id = :perfilBolsaAgua ").append("      and ci.clim_icnomeconta = :sim ")
					...
					.append(" and clie_icrecusasubsidio = :nao ").append(" and r.rota_id = :idRota");

			idsImoveis = (List<Integer>) session.createSQLQuery(consulta.toString())
					.addScalar("idImovel", Hibernate.INTEGER).setShort("sim", ConstantesSistema.SIM)
					.setShort("nao", ConstantesSistema.NAO).setInteger("idRota", idRota)
					.setInteger("perfilBolsaAgua", ImovelPerfil.BOLSA_AGUA).list();
	...
	public void atualizarPerfilImovel(Integer idImovel, Integer idPerfil) throws ErroRepositorioException {
		...
		try {
			...
			session.createQuery(update).setInteger("idImovel", idImovel).setInteger("idPerfil", idPerfil)
					.setTimestamp("ultimaAlteracao", new Date()).executeUpdate();
