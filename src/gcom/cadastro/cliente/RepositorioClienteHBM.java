...
public class RepositorioClienteHBM implements IRepositorioCliente {
	...
	public Collection pesquisarResponsavelSuperior(FiltroCliente filtroCliente) throws ErroRepositorioException {
		...
		try {
			...
			retorno = new ArrayList(new CopyOnWriteArraySet(GeradorHQLCondicional.gerarCondicionalQuery(filtroCliente, "cliente",
					"from gcom.cadastro.cliente.Cliente as cliente", session).list()));
	...
	public void removerTodosTelefonesPorCliente(Integer idCliente) throws ErroRepositorioException {
		...
		try {
			Iterator iterator = session.createQuery("from gcom.cadastro.cliente.ClienteFone where cliente = :cliente")
	...
	public void removerTodosEnderecosPorCliente(Integer idCliente) throws ErroRepositorioException {
		...
		try {
			Iterator iterator = session.createQuery("from gcom.cadastro.cliente.ClienteEndereco where cliente = :cliente")
					.setInteger("cliente", idCliente.intValue()).iterate();
	...
	public Collection<Cliente> pesquisarClienteDadosClienteEndereco(FiltroCliente filtroCliente, Integer numeroPagina) throws ErroRepositorioException {
		...
		try {
			retorno = new ArrayList(new CopyOnWriteArraySet<Cliente>(GeradorHQLCondicional
					.gerarCondicionalQuery(filtroCliente, "cliente",
							"select distinct cliente from gcom.cadastro.cliente.Cliente as cliente " + "left join cliente.clienteEnderecos clienteEndereco",
							//
	...
	public Integer pesquisarClienteDadosClienteEnderecoCount(FiltroCliente filtroCliente) throws ErroRepositorioException {
		...
		try {
			...
			Query query = GeradorHQLCondicional.gerarCondicionalQuery(filtroCliente, "cliente",
					"select count(distinct cliente) from gcom.cadastro.cliente.Cliente as cliente " + "left join cliente.clienteEnderecos clienteEndereco "
							+ "left join cliente.clienteImoveis clienteImovel", session);
	...
	public Cliente pesquisarCliente(Integer idCliente) throws ErroRepositorioException {
		...
		String consulta = null;
		try {
			consulta = "select cliente from Cliente cliente " + "INNER JOIN FETCH cliente.clienteTipo " + "where cliente.id = :idCliente";
	...
	public Object[] pesquisarQtdeImoveisEEconomiasCliente(Integer idCliente) throws ErroRepositorioException {
		...
		try {
			// cria o HQL para consulta
			String consulta = "SELECT count(imov.imov_id) as qtdeImoveis, " + " sum(imov.imov_qteconomia) as qtdeEconomias " + " FROM cadastro.imovel imov "
	...
	public Collection<ClienteFone> pesquisarClienteFone(Integer idCliente) throws ErroRepositorioException {
		...
		try {
			consulta = "from ClienteFone cfon " + "left join fetch cfon.cliente clie " + "left join fetch cfon.foneTipo fnet " + "where clie.id = :idCliente ";
	...
	public Collection<ClienteFone> pesquisarClienteFoneDoImovel(Integer idImovel) throws ErroRepositorioException {
		...
		try {
			consulta = "select cfon " + "from ClienteFone cfon, " + " ClienteImovel clienteImovel " + "left join fetch cfon.foneTipo fnet "
					+ "where clienteImovel.cliente.id = cfon.cliente.id " + "and clienteImovel.imovel.id = :idImovel ";
	...
	public Object[] pesquisarClienteFonePrincipal(Integer idCliente) throws ErroRepositorioException {
		...
		try {
			consulta = "SELECT cf.ddd, cf.telefone, cf.ramal " + "FROM ClienteFone cf " + "INNER JOIN cf.cliente cli " + "WHERE cli.id = :idCliente AND "
					+ "cf.indicadorTelefonePadrao = " + ClienteFone.INDICADOR_FONE_PADRAO;
	...
	public Collection pesquisarClienteEmitirBoletimCadastro(Integer idImovel, Short clienteRelacaoTipo) throws ErroRepositorioException {
		...
		try {
			consulta = "SELECT clie.clie_id as id, clie.clie_nmcliente as nome, clie.cltp_id as tipo, " // 0,
					...
					+ " and clim_dtrelacaofim is null " + "ORDER BY cf.cfon_icfonepadrao ";
			...
			retorno = (Collection) session.createSQLQuery(consulta).addScalar("id", Hibernate.INTEGER).addScalar("nome", Hibernate.STRING)
					...
					.addScalar("rg", Hibernate.STRING).addScalar("dataEmissaoRg", Hibernate.DATE).addScalar("orgaoExpedidorRg", Hibernate.STRING)
					.addScalar("uf", Hibernate.STRING).addScalar("dataNascimento", Hibernate.DATE).addScalar("profissao", Hibernate.STRING)
					.addScalar("sexo", Hibernate.INTEGER).addScalar("nomeMae", Hibernate.STRING).addScalar("icUso", Hibernate.SHORT)
					.addScalar("tipoEndereco", Hibernate.INTEGER).addScalar("idLogradouro", Hibernate.INTEGER).addScalar("cep", Hibernate.INTEGER)
					.addScalar("bairro", Hibernate.INTEGER).addScalar("idEnderecoReferencia", Hibernate.INTEGER).addScalar("numeroImovel", Hibernate.STRING)
					.addScalar("complemento", Hibernate.STRING).addScalar("idFoneTipo", Hibernate.INTEGER).addScalar("ddd", Hibernate.STRING)
					.addScalar("numeroTelefone", Hibernate.STRING).addScalar("ramal", Hibernate.STRING).addScalar("municipio", Hibernate.INTEGER)
					.setInteger("idImovel", idImovel).setShort("clienteRelacaoTipo", clienteRelacaoTipo).setMaxResults(2).list();
	...
	public Collection filtrarCliente(String codigo, String cpf, String rg, String cnpj, String nome, String nomeMae, String cep, String idMunicipio,
		...
		try {
			...
			// esfera poder
			if ((idEsferaPoder != null && !idEsferaPoder.equals("") && !idEsferaPoder.trim().equalsIgnoreCase(
					new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO).toString()))) {
				consulta = consulta + " left join clienteTipo.esferaPoder esferaPoder ";
			}

			// cep
			if (cep != null && !cep.equals("") && !cep.trim().equalsIgnoreCase(new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO).toString())) {
				consulta = consulta + " left join cliente.clienteEnderecos clienteEnderecos " + " left join clienteEnderecos.logradouroCep logradouroCep "
						+ "left join logradouroCep.cep cep ";
			}
			// bairro
			if (idBairro != null && !idBairro.equals("") && !idBairro.trim().equalsIgnoreCase(new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO).toString())) {
				consulta = consulta + " left join cliente.clienteEnderecos clienteEnderecos "
						+ " left join clienteEnderecos.logradouroBairro logradouroBairro " + " left join logradouroBairro.bairro bairro ";
			}
			...
			if (idEsferaPoder != null && !idEsferaPoder.equals("")
					&& !idEsferaPoder.trim().equalsIgnoreCase(new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO).toString())) {
			...
			if (idEsferaPoder != null && !idEsferaPoder.equals("")
					&& !idEsferaPoder.trim().equalsIgnoreCase(new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO).toString())) {
				query.setInteger("idEsferaPoder", new Integer(idEsferaPoder).intValue());
			}
	...
	public Object filtrarQuantidadeCliente(String codigo, String cpf, String rg, String cnpj, String nome, String nomeMae, String cep, String idMunicipio,
		...
		try {
			...
			consulta = "select count(distinct cliente.id) " + "from Cliente cliente " + " left join cliente.clienteTipo clienteTipo ";
	...
	public void atualizarCPFCliente(String cpf, Integer idCliente) throws ErroRepositorioException {
		...
		try {
			...
			update = "update gcom.cadastro.cliente.Cliente " + "set clie_nncpf = :cpf,clie_tmultimaalteracao = :ultimaAlteracao "
	...
	public Object[] obterDadosCliente(Integer idImovel, Short idClienteRelacaoTipo) throws ErroRepositorioException {
		...
		try {
			...
			consulta = " select cliente.clie_id as idCliente, "
					...
					" clie_nnrg as rg, "
					+ // 5
					" clie_dtrgemissao as dtRgEmissao, "
					+ // 6
					" oerg_dsabreviado as orgaoExp, "
					+ // 7
					" unfe_dsufsigla as unidFederativa, "
					+ // 8
					" clie_dtnascimento as dtNascimento, "
					+ // 9
					" prof_id as idProfissao, "
					+ // 10cltp_id
					" psex_id as idSexo, "
					+ // 11
					" clie_nnmae as nomeMae, "
					+ // 12
					" clie_icuso as indUso, "
					+ // 13
					" clie_dsemail as email, "
					+ // 14
					" edtp_id as idEnderecoTipo, "
					+ // 15
					...
					+ // 20
					" edrf_id as idEnderecoReferencia, "
					+ // 21
					" cled_nnimovel as numero, "
					+ // 22
					" cled_dscomplementoendereco as complemento, "
					+ // 23
					...
					+ " left join cadastro.unidade_federacao unfe on(cliente.unfe_id = unfe.unfe_id)" + " where clienteImovel.imov_id = :idImovel "
					+ " and clienteImovel.crtp_id = :idClienteRelacaoTipo " + " and clienteImovel.clim_dtrelacaofim is null"
					+ " order by clie_cdclienteresponsavel desc";

			retornoConsulta = (Object[]) session.createSQLQuery(consulta).addScalar("idCliente", Hibernate.INTEGER).addScalar("nomeCliente", Hibernate.STRING)
					.addScalar("idClienteTipo", Hibernate.INTEGER).addScalar("cpf", Hibernate.STRING).addScalar("cnpj", Hibernate.STRING)
					.addScalar("rg", Hibernate.STRING).addScalar("dtRgEmissao", Hibernate.DATE).addScalar("orgaoExp", Hibernate.STRING)
					.addScalar("unidFederativa", Hibernate.STRING).addScalar("dtNascimento", Hibernate.DATE).addScalar("idProfissao", Hibernate.INTEGER)
					.addScalar("idSexo", Hibernate.INTEGER).addScalar("nomeMae", Hibernate.STRING).addScalar("indUso", Hibernate.SHORT)
					.addScalar("email", Hibernate.STRING).addScalar("idEnderecoTipo", Hibernate.INTEGER).addScalar("cepLogradouro", Hibernate.INTEGER)
	...
	public Collection obterDadosClienteFone(Integer idCliente) throws ErroRepositorioException {
		...
		try {
			...
			consulta = " select clie_id as idCliente, " + // 0
					...
					" from cadastro.cliente_fone" + " where clie_id = :idCliente";

			retornoConsulta = session.createSQLQuery(consulta).addScalar("idCliente", Hibernate.INTEGER).addScalar("idClienteFone", Hibernate.INTEGER)
					.addScalar("ddd", Hibernate.STRING).addScalar("telefone", Hibernate.STRING).addScalar("ramal", Hibernate.STRING)
					.addScalar("foneTipo", Hibernate.INTEGER).addScalar("icFonePadrao", Hibernate.SHORT).setInteger("idCliente", idCliente).list();
	...
	public Integer verificaExistenciaClienteAtualizacaoCadastral(Integer idCliente) throws ErroRepositorioException {
		...
		try {
			...
			consulta = "select clie_id as id " + "from cadastro.cliente_atlz_cadastral " + "where clie_id = :idCliente  ";

			retornoConsulta = (Integer) session.createSQLQuery(consulta).addScalar("id", Hibernate.INTEGER).setInteger("idCliente", idCliente).setMaxResults(1)
					.uniqueResult();
	...
	public Integer pesquisarClienteResponsavelSuperiorParaPaginacaoCount(PesquisarClienteResponsavelSuperiorHelper helper) throws ErroRepositorioException {
		...
		try {
			...
			consulta = "SELECT COUNT (DISTINCT clienteSuperior) " + " FROM Cliente cliente " + " INNER JOIN cliente.cliente clienteSuperior "
	...
	public Collection<Cliente> pesquisarClienteResponsavelSuperiorParaPaginacao(PesquisarClienteResponsavelSuperiorHelper helper, Integer numeroPagina)
		...
		try {
			...
			consulta = "SELECT DISTINCT clienteSuperior " + " FROM Cliente cliente " + " INNER JOIN cliente.cliente clienteSuperior "
					+ " LEFT JOIN clienteSuperior.clienteTipo clienteSuperiorTipo ";
	...
	public String criarCondicionaisPesquisarClienteResponsavelSuperior(PesquisarClienteResponsavelSuperiorHelper helper) {
		...
		if (helper.getNome() != null && !helper.getNome().trim().equals("")) {
			if (helper.getTipoPesquisa() != null && helper.getTipoPesquisa().equals(ConstantesSistema.TIPO_PESQUISA_COMPLETA)) {
	...
	public IClienteAtualizacaoCadastral pesquisarClienteAtualizacaoCadastral(Integer idCliente, Integer idImovel, Integer idClienteRelacaoTipo)
		...
		try {
			...
			consulta = " SELECT clie" + " FROM ClienteAtualizacaoCadastral clie" + " WHERE clie.idImovel = :idImovel ";
			...
			clienteAtualizacaoCadastral = (IClienteAtualizacaoCadastral) session.createQuery(consulta).setInteger("idImovel", idImovel).setMaxResults(1)
					.uniqueResult();
	...
	public Collection<ClienteFoneAtualizacaoCadastral> pesquisarClienteFoneAtualizacaoCadastral(Integer idCliente, Integer idMatricula, Integer idTipoFone,
			Integer idClienteRelacaoTipo, String numeroFone) throws ErroRepositorioException {
		...
		try {
			...
			consulta = " SELECT clienteFone" + " FROM ClienteFoneAtualizacaoCadastral clienteFone"
			...
			retorno = session.createQuery(consulta).setInteger("idCliente", idCliente.intValue()).setInteger("idMatricula", idMatricula).list();
	...
	public void atualizarTelefonePadrao(String idCliente, String idClienteFonePadrao) throws ErroRepositorioException {
		...
		try {
			...
			consulta = " UPDATE clienteFone" + " SET indicadorTelefonePadrao = 2" + " AND ultimaAlteracao = " + Util.obterSQLDataAtual()
					+ " WHERE cliente.id = :idCliente ";

			session.createQuery(consulta).setString("idCliente", idCliente).executeUpdate();

			// coloca no clienteFone a ser atualizado o indicadorTelefonePadrao
			// = 1
			consulta2 = " UPDATE clienteFone" + " SET indicadorTelefonePadrao = 1" + " WHERE cliente.id = :idCliente" + " AND id = :idClienteFonePadrao";
	...
	public Cliente pesquisarClienteUsuarioDoImovel(Integer idImovel) throws ErroRepositorioException {
		...
		try {
			// Monta o HQL
			consulta = " select cliente from ClienteImovel clim " + " INNER JOIN clim.cliente cliente " + " INNER JOIN clim.clienteRelacaoTipo crtp "
					+ " where clim.imovel.id = :idImovel " + " and crtp.id = :usuario " + " and clim.dataFimRelacao is null ";

			// Executa o HQL
			cliente = (Cliente) session.createQuery(consulta).setInteger("idImovel", idImovel).setInteger("usuario", ClienteRelacaoTipo.USUARIO).uniqueResult();
	...
	public Collection filtrarAutocompleteCliente(String valor) throws ErroRepositorioException {
		...
		try {
			...
			if (Util.validarStringNumerica(valor)) {
				...
				retorno = session.createSQLQuery(consulta).addScalar("id", Hibernate.INTEGER).addScalar("cnpj", Hibernate.STRING)
						.addScalar("nome", Hibernate.STRING).addScalar("ic", Hibernate.INTEGER).setMaxResults(200).list();
	...
	public Collection filtrarAutocompleteClienteResponsavel(String valor, ClienteTipo tipo) throws ErroRepositorioException {
		...
		try {
			consulta = " SELECT clie_id as id, clie_nmcliente as nome , clie_nncnpj as cnpj, cltp.cltp_icpessoafisicajuridica as ic from cadastro.cliente clie "
					+ "inner join cadastro.cliente_tipo cltp on cltp.cltp_id = clie.cltp_id where cltp.cltp_icpessoafisicajuridica="
					+ ClienteTipo.INDICADOR_PESSOA_JURIDICA + " and ";
	...
	public Integer pesquisarQtdClientesAssociadosResponsavelNaoJuridica(Integer idCliente) throws ErroRepositorioException {
		...
		try {
			...
			consulta = "SELECT count(*) as qtd FROM cadastro.cliente clie inner join cadastro.cliente_tipo cltp on cltp.cltp_id = clie.cltp_id "
					+ "WHERE clie.clie_CDCLIENTERESPONSAVEL = " + idCliente.intValue() + " and clie.clie_id <> " + idCliente.intValue() + " "
	...
	public String validarCliente(String cpfCliente, Integer matricula) throws ErroRepositorioException {
		...
		try {
			...
			sb.append("INNER JOIN cadastro.cliente_imovel imov ON imov.clie_id = cli.clie_id AND imov.imov_id = " + matricula
	...
	public Cliente pesquisarDadosCliente(Integer idCliente) throws ErroRepositorioException {
		...
		try {
			...
			dados = (Object[]) session.createSQLQuery(consulta).addScalar("nomeCliente", Hibernate.STRING).addScalar("nomeAbrevCliente", Hibernate.STRING)
					.addScalar("cpfCliente", Hibernate.STRING).addScalar("rgCliente", Hibernate.STRING).addScalar("dataRgEmissao", Hibernate.DATE)
					.addScalar("dataVencimento", Hibernate.SHORT).addScalar("cnpjCliente", Hibernate.STRING).addScalar("emailCliente", Hibernate.STRING)
					.addScalar("clienteResp", Hibernate.INTEGER).addScalar("idClienteTipo", Hibernate.INTEGER).addScalar("descClienteTipo", Hibernate.STRING)
					.addScalar("indicadorUsoClienteTipo", Hibernate.SHORT).addScalar("indicadorPJ", Hibernate.SHORT).setInteger("idCliente", idCliente)
	...
	public ClienteTipo pesquisarClienteTipo(Integer idClienteTipo) throws ErroRepositorioException {
		...
		try {
			...
			consulta = "select cltp from ClienteTipo cltp " + "where cltp.id = :idClienteTipo ";

			retorno = (ClienteTipo) session.createQuery(consulta).setInteger("idClienteTipo", idClienteTipo).setMaxResults(1).uniqueResult();
	...
	public Collection<Cliente> pesquisarClientePorCpfCnpj(String cpfCnpj) throws ErroRepositorioException {
		...
		try {
			...
			retorno = (Collection<Cliente>) session.createQuery(consulta).setString("cpfCnpj", cpfCnpj).list();
		} catch (HibernateException e) {
			throw new ErroRepositorioException("Erro no Hibernate");
		} finally {
			HibernateUtil.closeSession(session);
	...
	public Integer pesquisarEnderecoClienteParaNegativacao(Integer idCliente) throws ErroRepositorioException {
		...
		try {
			consulta = "SELECT cled.id " + " from ClienteEndereco cled " + " inner join  cled.logradouroCep as logCep " + " inner join  cled.cliente as clie "
					+ " inner join  logCep.cep " + " where clie.id = :idCliente "
					+ " and cled.indicadorEnderecoCorrespondencia = :indicadorEnderecoCorrespondencia ";

			retorno = (Integer) session.createQuery(consulta).setInteger("idCliente", idCliente)
					.setShort("indicadorEnderecoCorrespondencia", ConstantesSistema.SIM).setMaxResults(1).uniqueResult();
	...
	public Cliente pesquisarDadosClienteParaNegativacao(Integer idCliente, String cnpjEmpresa) throws ErroRepositorioException {
		...
		try {
			consulta = "SELECT cliente.id, " // 0
					...
					+ "from Cliente cliente " + "left join cliente.unidadeFederacao unfe "
					+ "where cliente.id = :idCliente "
					+ "and (cliente.cnpj is not null or cliente.cpf is not null)" + "and (cliente.cnpj is null or cliente.cnpj <>:cnpjEmpresa)";

			arrayCliente = (Object[]) session.createQuery(consulta).setInteger("idCliente", idCliente).setString("cnpjEmpresa", cnpjEmpresa).setMaxResults(1)
					.uniqueResult();
	...
	public String obterNomeClienteConta(Integer idImovel) throws ErroRepositorioException {
		...
		try {
			consulta.append("SELECT cliente.nome FROM ClienteImovel clienteImovel ")
					...
					.append("AND  clienteImovel.indicadorNomeConta = :ativo ");

			retorno = (String) session.createQuery(consulta.toString())
							.setInteger("idImovel", idImovel)
							.setShort("ativo", ConstantesSistema.SIM).uniqueResult();
