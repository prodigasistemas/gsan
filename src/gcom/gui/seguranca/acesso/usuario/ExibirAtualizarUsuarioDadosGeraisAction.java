...
public class ExibirAtualizarUsuarioDadosGeraisAction extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest request, HttpServletResponse response) {
		...
		if (usuarioParaAtualizar != null) {
			...
			if ("".equals(form.getIdFuncionario())) {
				...
				if (usuarioParaAtualizar.getFuncionario() != null) {
					...
					&& (usuarioParaAtualizar.getFuncionario() == null || (usuarioParaAtualizar.getFuncionario() != null && usuarioParaAtualizar.getFuncionario().getId() != null && !form
							.getIdFuncionario().equals(usuarioParaAtualizar.getFuncionario().getId().toString())))) {
			...
			if ("".equals(form.getIdLotacao())) {
				...
				if (usuarioParaAtualizar != null && usuarioParaAtualizar.getUsuarioTipo() != null && usuarioParaAtualizar.getUsuarioTipo().getId() != null
					...
					&& ((usuarioParaAtualizar.getUnidadeOrganizacional() == null) || (usuarioParaAtualizar.getUnidadeOrganizacional() != null && usuarioParaAtualizar.getUnidadeOrganizacional()
							.getId() != null))) {
				...
				if (coll != null && !coll.isEmpty()) {
					...
					if (colecaoUsuarioGrupo == null || colecaoUsuarioGrupo.isEmpty()) {
						...
						// efetuando seja diferente da unidade de lotação
						// informada
								...
								while (mesmoNivel) {
									...
									if (unidadeEmpresaSuperior != null) {
										...
										if (unidadeEmpresaSuperior.getUnidadeTipo().getNivel().intValue() == idNivelUsuario) {
											...
											// da unidade empresa do usuário no
											// mesmo nivel
		...
		if (usuarioParaAtualizar.getLimiteBatch() != null)
			form.setLimiteBatch(usuarioParaAtualizar.getLimiteBatch().toString());
		...
		if (sessao.getAttribute("collUsuarioTipo") == null || !sessao.getAttribute("collUsuarioTipo").equals("")) {
			...
			if (!usuario.getUsuarioTipo().getId().equals(UsuarioTipo.USUARIO_TIPO_ADMINISTRADOR)) {
				...
				filtroUsuarioTipo.adicionarParametro(new ParametroSimplesDiferenteDe(FiltroUsuarioTipo.ID, UsuarioTipo.USUARIO_TIPO_ADMINISTRADOR));
			}

			filtroUsuarioTipo.adicionarParametro(new ParametroSimples(FiltroUsuarioTipo.INDICADOR_USO, ConstantesSistema.INDICADOR_USO_ATIVO));
		...
		if (sessao.getAttribute("collUsuarioAbrangencia") == null || sessao.getAttribute("collUsuarioAbrangencia").equals("")) {
			...
			if (colecaoUsuarioGrupo != null && !colecaoUsuarioGrupo.isEmpty()) {
				...
				colecaoUsuarioAbrangencia = Fachada.getInstancia().pesquisar(new FiltroUsuarioAbrangencia(), UsuarioAbrangencia.class.getSimpleName());
			} else {
				...
				while (mesmoNivel) {
					Collection colecaoUsuarioAbrangenciaPorSuperior = Fachada.getInstancia().pesquisarUsuarioAbrangenciaPorSuperior(usuarioAbrangenciaParaPesquisa, idUsuarioAbrangencia);
					if (colecaoUsuarioAbrangenciaPorSuperior != null && !colecaoUsuarioAbrangenciaPorSuperior.isEmpty()) {
						...
						// abrangências o usuario abrangência do usuario
						// atualizar
				...
				if (!achouUsuarioAbrangencia) {
					...
					filtroUsuarioAbrangencia.adicionarParametro(new ParametroSimples(FiltroUsuarioAbrangencia.ID, usuarioParaAtualizar.getUsuarioAbrangencia().getId()));
		...
		if (sessao.getAttribute("collGerenciaRegional") == null || sessao.getAttribute("collGerenciaRegional").equals("")) {
			...
			Collection colecaoGerenciaRegional = Fachada.getInstancia().pesquisar(filtroGerenciaRegional, GerenciaRegional.class.getSimpleName());
		...
		if (sessao.getAttribute("collUnidadeNegocio") == null || sessao.getAttribute("collUnidadeNegocio").equals("")) {
			...
			Collection colecaoUnidadeNegocio = Fachada.getInstancia().pesquisar(filtroUnidadeNegocio, UnidadeNegocio.class.getSimpleName());
		...
		if (sessao.getAttribute("collUsuarioSituacao") == null || sessao.getAttribute("collUsuarioSituacao").equals("")) {
			...
			filtroUsuarioSituacao.adicionarParametro(new ParametroSimples(FiltroUsuarioSituacao.INDICADOR_DE_USO, ConstantesSistema.INDICADOR_USO_ATIVO));

			Collection colecaoUsuarioSituacao = Fachada.getInstancia().pesquisar(filtroUsuarioSituacao, UsuarioSituacao.class.getSimpleName());
		...
		// IDADE MÍNIMA
		request.setAttribute("idadeMinimaFuncionario", ConstantesSistema.IDADE_MINIMA_FUNCIONARIO);
		request.setAttribute("idadeMinimaUsuario", ConstantesSistema.IDADE_MINIMA_USUARIO);
