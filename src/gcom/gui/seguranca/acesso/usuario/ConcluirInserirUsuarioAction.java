...
public class ConcluirInserirUsuarioAction extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest request, HttpServletResponse response) {
			...
			if (coll != null && !coll.isEmpty()) {
				...
				if (!usuarioTipo.getId().equals(UsuarioTipo.USUARIO_TIPO_ADMINISTRADOR)) {
					...
					if (usuarioTipo.getIndicadorFuncionario() == UsuarioTipo.INDICADOR_FUNCIONARIO) {
						...
						if (form.getIdFuncionario() != null && (form.getNomeFuncionario() == null || form.getNomeFuncionario().equals(""))) {
						...
						// Data de Nascimento do funcionario é obrigatoria para
						// usuários diferentes de rotina batch e/ou internet
						if (form.getIndicadorUsuarioBatch() != 1 && form.getIndicadorUsuarioInternet() != 1) {
						...
						if (usuarioTipo.getIndicadorFuncionario() != UsuarioTipo.INDICADOR_FUNCIONARIO) {
							...
							// CPF do funcionario é obrigatorio para usuários
							// diferentes de rotina batch e/ou internet
							if (form.getIndicadorUsuarioBatch() != 1 && form.getIndicadorUsuarioInternet() != 1) {
								if (form.getCpf() == null || form.getCpf().equals("")) {
									throw new ActionServletException("atencao.required", null, "Número do CPF");
							...
							// Data de Nascimento do funcionario é obrigatoria
							// para usuários diferentes de rotina batch e/ou
							// internet
							if (form.getIndicadorUsuarioBatch() != 1 && form.getIndicadorUsuarioInternet() != 1) {
								if (form.getDataNascimento() == null || form.getDataNascimento().equals("")) {
									throw new ActionServletException("atencao.required", null, "Data de Nascimento");
		...
		// Não adiciona os grupos em caso de seleção do indicador batch e/ou
		// internet
		...
		montarPaginaSucesso(request, "Usuário de Login " + usuarioCadastrar.getLogin() + " inserido com sucesso!", "Inserir outro Usuário", "exibirInserirUsuarioAction.do?menu=sim");
