		...
		try {
			...
			if (dbcb != null) {
				if (indicadorDiscriminarDescricao == 1) {
					...
					dados[2] = Util.formatarMoedaReal(soma);
					/**
					 * Contas de rotas do interior que não possuiam a descrição
					 * dos débitos
					 * 
	...
	public String gerarArquivoFormatadoImpressaoTermica(EmitirContaHelper emitirContaHelper, SistemaParametro sistemaParametro, 
		...
		try {
			...
			MedicaoHistorico medicaoHistoricoAguaMesAnterior = getControladorMicromedicao().pesquisarMedicaoHistoricoTipoAguaLeituraAnormalidade(
					imovelEmitido.getId(), Util.subtrairMesDoAnoMes(emitirContaHelper.getAmReferencia(), 1));
			...
			try {
				...
				try {
					...
					String situacaoAgua = emitirContaHelper.getDescricaoLigacaoAguaSituacao();
					String situacaoEsgoto = emitirContaHelper.getDescricaoLigacaoEsgotoSituacao();
					;
					...
					if (medicaoHistoricoAgua != null) {
						...
						if (medicaoHistoricoAgua.getDataLeituraAnteriorFaturamento() != null) {
							dataLeituraAnteriorFaturada = Util.formatarData(medicaoHistoricoAgua.getDataLeituraAnteriorFaturamento());
						}
						...
						if (consumoAgua != null) {
							...
							if (medicaoHistoricoAgua.getLeituraAtualFaturamento() != 0) {
								...
								diasConsumo = Util.obterQuantidadeDiasEntreDuasDatas(medicaoHistoricoAgua.getDataLeituraAnteriorFaturamento(),
										medicaoHistoricoAgua.getDataLeituraAtualFaturamento()) + "";
								leituraAtualFaturada = medicaoHistoricoAgua.getLeituraAtualFaturamento() + "";
							} else {

								consumo = consumoAgua.getNumeroConsumoFaturadoMes() + "";
								diasConsumo = Util.obterQuantidadeDiasEntreDuasDatas(medicaoHistoricoAgua.getDataLeituraAtualFaturamento(),
										medicaoHistoricoAgua.getDataLeituraAnteriorFaturamento()) + "";
							}
						}
						...
						if (medicaoHistoricoPoco.getDataLeituraAnteriorFaturamento() != null) {
							dataLeituraAnteriorFaturada = Util.formatarData(medicaoHistoricoPoco.getDataLeituraAnteriorFaturamento());
						}
						...
						if (consumoEsgoto != null) {
							...
							if (medicaoHistoricoPoco.getLeituraAtualFaturamento() != 0) {
								...
								diasConsumo = Util.obterQuantidadeDiasEntreDuasDatas(medicaoHistoricoPoco.getDataLeituraAtualFaturamento(),
										medicaoHistoricoPoco.getDataLeituraAnteriorFaturamento()) + "";
					...
					else if (medicaoHistoricoAgua == null && medicaoHistoricoPoco == null) {
					...
					retorno.append(formarLinha(7, 0, 248, 301, dataInstalacao, 0, 0));
					// Situacao da ligacao de Agua

					if (situacaoAgua.length() > 13) {
						retorno.append(formarLinha(7, 0, 446, 301, situacaoAgua.substring(0, 13), 0, 0));
					} else {
						retorno.append(formarLinha(7, 0, 446, 301, situacaoAgua, 0, 0));
					}
					// Situacao da ligacao de esgoto
					retorno.append(formarLinha(7, 0, 627, 301, situacaoEsgoto, 0, 0));

					// Leitura Anterior
					retorno.append(formarLinha(7, 0, 188, 330, "LEITURA", 0, 0) + formarLinha(7, 0, 190, 354, leituraAnteriorInformada, 0, 0)
							+ formarLinha(7, 0, 190, 378, leituraAtualInformada, 0, 0));
					retorno.append(formarLinha(7, 0, 320, 330, "DATA", 0, 0) + formarLinha(7, 0, 298, 354, dataLeituraAnteriorInformada, 0, 0)
					...
					if (medicaoHistoricoAgua != null) {
						if (medicaoHistoricoAgua.getLeituraAnormalidadeFaturamento() != null) {
							retorno.append(formarLinha(0, 2, 460, 374, "ANORM. LEITURA: "
					...
					// Consumo
					if ((imovelEmitido.getLigacaoAguaSituacao().getId() == LigacaoAguaSituacao.LIGADO || imovelEmitido.getLigacaoEsgotoSituacao().getId() == LigacaoEsgotoSituacao.LIGADO)
					...
					retorno.append(formarLinha(7, 0, 53, 708, "DESCRICAO", 0, 0) + formarLinha(7, 0, 571, 708, "CONSUMO", 0, 0)
							+ formarLinha(7, 0, 687, 708, "TOTAL(R$)", 0, 0));
					...
					for (int i = 0; i < debitos.size(); i++) {
						...
						if (debito[1] != null) {
							retorno.append(formarLinha(7, 0, 571, 733, debito[1], 0, (i + 1) * 34 + ultimaLinhaPoco));
						}
						if (debito[2] != null) {
							retorno.append(formarLinha(7, 0, 697, 733, debito[2], 0, (i + 1) * 34 + ultimaLinhaPoco));
						}
					}
					...
					retorno.append(formarLinha(0, 2, 344, 1456, emitirContaHelper.getIdImovel() + "", 0, 0)
							...
							+ formarLinha(0, 2, 694, 1456, Util.formatarMoedaReal(valorTotalConta), 0, 0));
